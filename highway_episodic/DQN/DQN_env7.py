...
class rlEnv():
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if('ego' in vehs):  
            ...
            print('TTC: ',TTC)
            traci.vehicle.subscribeContext('ego', tc.CMD_GET_VEHICLE_VARIABLE, 60.0, [tc.VAR_POSITION])
            # print(id,' subscribeContext')
            ...
            if action == 0: 
                ...
                if TTC >3:
                    # a_des1 = 2*(s1-v_ego*(TTC-3))/(TTC-3)**2
                    a_des1 = s1/(TTC-3)*accel_weight
                else:
                    a_des1 = 3
                print('a_des1: ',a_des1) 
                self.__set_ego_v('ego',ego_v_last +a_des1)                
            elif action == 2:
                if TTC >3:
                    # a_des2 = 2*(s2-v_ego*(TTC-3))/(TTC-3)**2
                    a_des2 = s2/(TTC-3)*accel_weight
                else:
                    a_des2 = 3   
                print('a_des2: ',a_des2)              
                self.__set_ego_v('ego',ego_v_last +a_des2)
            elif action == 3: #s3 <0
                if TTC >3:
                    # a_des3 = 2*(s3-v_ego*(TTC-3))/(TTC-3)**2
                    a_des3 = s3/(TTC-3)*accel_weight
                else:
                    a_des3 = -5
                print('a_des3: ',a_des3)                
                self.__set_ego_v('ego',ego_v_last +a_des3)
            elif action == 4: #s4 <0
                if TTC >3:
                    # a_des4 = 2*(s4-v_ego*(TTC-3))/(TTC-3)**2
                    a_des4 = s4/(TTC-3)*accel_weight
                else:
                    a_des4 = -5 
                print('a_des4: ',a_des4) 
                self.__set_ego_v('ego',ego_v_last +a_des4)
