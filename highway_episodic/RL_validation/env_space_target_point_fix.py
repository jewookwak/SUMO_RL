...
class rlEnv():
    ...
    def reset(self):
        ...
        rlEnv.vehicles_tau=[1.36]*15 ########### RL 검증
    ...
    def save_csv(self):
        ...
        os.chdir('/home/mds/Desktop/highway_episodic/DQN/RL_validation/data_log/1')
    ...
    def __ego_vehicle_LC_completed(self):
        ...
        if 'ego' in vehs:
            if rlEnv.ego_LC_start == True:
                if (traci.vehicle.getLaneIndex('ego')== 2 or traci.vehicle.getLaneIndex('ego')== 0) and traci.vehicle.getAngle('ego') >= 89.9 and traci.vehicle.getAngle('ego') <= 90.1 and np.abs(traci.vehicle.getLateralLanePosition('ego'))<=0.5:                    
        ...
        if 'ego' in vehs:
            ...
            if lane == 0 and LLP >=0:
                if action == 5:
                    ...
                    if len(left_vehicles)<=1:
                        left_target_space = [traci.vehicle.getPosition('ego')[0],-4.8]
                        vehicles = ['ego','ego'] #...
                    else:
                        for i in range(2):
                            left_near_vehicles.append([left_vehicles[:3][i][1],left_vehicles[:3][i][2]])# 1: left vehicle position 2: left vehicle id
            ...
            if lane == 1 and LLP<0:
                if action == 5:
                    ...
                    left_vehicles.sort()
                    if len(left_vehicles)<=1:
                        left_target_space = [traci.vehicle.getPosition('ego')[0]+5,-4.8]
                        vehicles = ['ego','ego']
                    else:
                        for i in range(2):
                            left_near_vehicles.append([left_vehicles[:3][i][1],left_vehicles[:3][i][2]])# 1: left vehicle position 2: left vehicle id
            ...
            if lane == 1 and LLP>0:
                if action == 5:
                    ...
                    if len(right_vehicles)<=1:
                        right_target_space = [traci.vehicle.getPosition('ego')[0],-4.8]
                        vehicles = ['ego','ego']
                    else:
                        for i in range(2):
                            right_near_vehicles.append([right_vehicles[:3][i][1],right_vehicles[:3][i][2]])# 1: right vehicle position 2: right vehicle id
    ...
    def __set_ego_a(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if(a_cur+jerk*dt<a_des):
            ...
            traci.vehicle.setAcceleration(id,a_des,1)
            return a_des
    def __set_ego_a_y_desired(self,id):
        jerk = 5
    ...
    def __set_ego_a_movetoXY(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if(a_y_cur+jerk*dt<a_y_des):
            v_y_next = v_y_cur+(a_y_cur+jerk*dt)*dt
...
########################################################
    ...
    def __toward_biggest_empty_space_reward(self,id,action):
        ...
        if id in vehs and rlEnv.Target_left_car in vehs and rlEnv.Target_right_car in vehs:        
            ...
            SD_LC_s1 = np.abs((traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + self.SD_LC_with_respect_to_target_space('ego',1)) - traci.vehicle.getPosition('ego')[0])
            SD_LC_s2 = np.abs((traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + self.SD_LC_with_respect_to_target_space('ego',2)) - traci.vehicle.getPosition('ego')[0])
            SD_LC_s3 = np.abs((traci.vehicle.getPosition(rlEnv.Target_left_car)[0] - self.SD_LC_with_respect_to_target_space('ego',1)) - traci.vehicle.getPosition('ego')[0])
            SD_LC_s4 = np.abs((traci.vehicle.getPosition(rlEnv.Target_right_car)[0] - self.SD_LC_with_respect_to_target_space('ego',1)) - traci.vehicle.getPosition('ego')[0])
            #distance to space
            ...
            #space의 SD_LC 까지 거리
            SD_LC_space1 = [space1_size,SD_LC_s1,'L1'] #left front
            SD_LC_space2 = [space2_size,SD_LC_s2,'R1'] #right front
            SD_LC_space3 = [space3_size,SD_LC_s3,'L2'] #left back
            SD_LC_space4 = [space4_size,SD_LC_s4,'R2'] #right back
            ...
            second_biggest_size_value = 0.25
            third_biggest_size_value = 0.25
    ...
    #             return-50  
    def SD_LC_with_respect_to_target_space(self,id,action): #action 1~4 = target space 1~4, action 에 따라 SD_LC를 return
        id = 'ego'
        time_gap_LC_1 =1
        time_gap_LC_2 =0.5 
        ...
        if rlEnv.Target_left_car in vehs and rlEnv.Target_right_car in vehs and 'ego' in vehs:  
            if action == 1:
                ...
                SD_LC = relative_velocity_term + minimum_clearance_trem
                ...
                return  min([SD_LC, space1_size/2])
            elif action == 2:
                ...
                return  min([SD_LC, space2_size/2])
            elif action == 3:
                ...
                return min([SD_LC, space3_size/2])
            elif action == 4:
                ...
                return min([SD_LC, space4_size/2])
            else:
                return 0
    ...
    def __reward(self, step_num, action):
        ...
        w3 = 3 # distance to biggest empty space positive reward
        ...
        if action == 7 and Lane == 1 and np.abs(LLP)<=0.56 and (rear_TTC<2.4  and (rlEnv.Left_SD == False and rlEnv.Right_SD == False))and rlEnv.Front_SD:
            ...
            if('ego' in vehs and 'accel.rear' in vehs):           
                ...
                if(rear_TTC<=13.2 and rear_clearance>=0): # 차로 변경을 위한 가장큰 빈공간의 중앙에 가까워질 수 록 보상을 준다.
                    ...
                    reward += self.__toward_biggest_empty_space_reward('ego',action)*w3
                    Reward.append(self.__toward_biggest_empty_space_reward('ego',action)*w3)
                else:
                    Reward.append(0)
                    reward +=0
    ...
    def stanley(self,coef,heading_angle,x_cur,v_cur):
        ...
        delta = (90-heading_angle)*np.pi/180
        x = symbols('x')
    ...
    def LC_Left(self,action):
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                if(traci.vehicle.getLaneIndex('ego')==2 and traci.vehicle.getLateralLanePosition('ego')>=0.5):
                    print('stop moveToXY')
                    self.done = True
                else:
    ...
    def LC_Right(self, action):
        ...
        if('ego' in vehs):  
            ...
            if action == 6:
                ...
                # if self.done != True and self.__ego_vehicle_LC_completed() !=True:
                if(traci.vehicle.getLaneIndex('ego')==0 and traci.vehicle.getLateralLanePosition('ego')<=-0.5):
                    print('stop moveToXY')
                    self.done = True
                else:
                    # print(':::::::::::::::::::::::::::::::::::movetoXY')
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if traci.simulation.getTime() == 0.03: # 기준이되는 옆차로 차량을 시뮬레이션 시작할 때 설정한다.(-> 후방 차량의 추돌 위험이 감지 될 때 마다 기준이 되는 옆차로 차량의 위치를 갱신하는게 맞다.)
            ...
            # print('Target_left_car: ',rlEnv.Target_left_car)
            # print('Target_right_car: ',rlEnv.Target_right_car)
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs): 
            ...
            if traci.vehicle.getLeader('ego') is not None:
                ...
                x_forward = traci.vehicle.getPosition(Leader_id)[0]-traci.vehicle.getPosition('ego')[0]
            ...
            time_gap_LK = 2.27  #95% percentile time_gap (most safe) //2.27 -> safe 1.36s -> 중간값   0.74s -> 5% persentile 가장 위험.
            v_p = traci.vehicle.getSpeed(Leader_id)
            des_clearance = c_LK + time_gap_LK*v_p #앞차 속도에 따른 앞차와의 ACC 안전거리 (safe)
            des_clearance2 = c_LK + time_gap_LC_prepare*v_p #앞차 속도에 따른 앞차와의 ACC 안전거리 (aggressive)
            if rear_relative_distance<=70:
                Anti_RVC_on = True
            else:
                Anti_RVC_on = False
            ...
            if rlEnv.Target_left_car in vehs and rlEnv.Target_right_car in vehs and 'ego' in vehs:
                ...
                s1 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + self.SD_LC_with_respect_to_target_space('ego',1)) - traci.vehicle.getPosition('ego')[0] #자차와 왼쪽 차로 전방 빈공간의 중점 까지 x축 방향 상대 거리
                s2 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + self.SD_LC_with_respect_to_target_space('ego',2)) - traci.vehicle.getPosition('ego')[0] #자차와 오른쪽 차로 전방 빈공간의 중점 까지 x축 방향 상대 거리
                s3 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] - self.SD_LC_with_respect_to_target_space('ego',3)) - traci.vehicle.getPosition('ego')[0] #자차와 왼쪽 차로 후방 빈공간의 중점 까지 x축 방향 상대 거리            
                s4 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] - self.SD_LC_with_respect_to_target_space('ego',4)) - traci.vehicle.getPosition('ego')[0] #자차와 오른쪽 차로 후방 빈공간의 중점 까지 x축 방향 상대 거리
                space1 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_leader)[0])/2
                space2 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_leader)[0])/2
                space3 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_follower)[0])/2
                space4 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_follower)[0])/2
            else:
                s1= rlEnv.S[1]
                s2= rlEnv.S[2]
                s3= rlEnv.S[3]
            ...
            # ss1 = (left_near_vehicles[1][0]+left_near_vehicles[2][0])/2 - traci.vehicle.getPosition('ego')[0]  #왼쪽 차로 가장 가까운 전방 빈공간의 중점 까지 x축 방향 거리
            # ss2 = (right_near_vehicles[1][0]+right_near_vehicles[2][0])/2 - traci.vehicle.getPosition('ego')[0] #오른쪽 차로 가장 가까운 전방 빈공간의 중점 까지 x축 방향 거리
            # ss3 = (left_near_vehicles[0][0]+left_near_vehicles[1][0])/2 - traci.vehicle.getPosition('ego')[0] #왼쪽 차로 가장 가까운 후방 빈공간의 중점 까지 x축 방향 거리
            # ss4 = (right_near_vehicles[0][0]+right_near_vehicles[1][0])/2 - traci.vehicle.getPosition('ego')[0] #오른쪽 차로 가장 가까운 후방 빈공간의 중점 까지 x축 방향 거리
            ...
            if action == 0: 
                ...
                if rlEnv.last_Space1 !=0:
                    v_t = (space1-rlEnv.last_Space1)/self.step_length
                ...
                if traci.vehicle.getLaneIndex('ego') == 0:
                    ...
                    if s1 >0:
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,0,s1,1))
                        
                        control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,0,s1,1),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Left_action = False     
                        rlEnv.ego_control.append(control)   
                    else:
                        self.LC_Left(action)
                ...
                if rlEnv.last_Space2 !=0:
                    v_t = (space2-rlEnv.last_Space2)/self.step_length
                ...
                if rlEnv.last_action[-1][1] != 2:
                    rlEnv.Right_action = False 
                if traci.vehicle.getLaneIndex('ego') == 2:
                    self.LC_Right(action)
                else:
                    if s2>0:   
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,0,s2,2))
                        control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,0,s2,2),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Right_action = False
                        rlEnv.ego_control.append(control)
                    else:
                        self.LC_Right(action)
                ...
                if rlEnv.last_Space3 !=0:
                    v_t = (space3-rlEnv.last_Space3)/self.step_length
                ...
                if traci.vehicle.getLaneIndex('ego') == 0:
                    ...
                    if s3<0:
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire2(v_ego,v_t,3,s3,3))
                        control = [traci.simulation.getTime(),self.__ego_a_desire2(v_ego,v_t,3,s3,3),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Left_action = False
                        rlEnv.ego_control.append(control)
                    else:
                        self.LC_Left(action)
            elif action == 4: #s4 <0
                if rlEnv.last_Space4 !=0:
                    v_t = (space4-rlEnv.last_Space4)/self.step_length
                ...
                if traci.vehicle.getLaneIndex('ego') == 0:
                    ...
                    if s4<0:
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire2(v_ego,v_t,3,s4,4))
                        control = [traci.simulation.getTime(),self.__ego_a_desire2(v_ego,v_t,3,s4,4),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Right_action = False
                        rlEnv.ego_control.append(control)
                    else:                    
                        self.LC_Right(action)
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs):
            ...
            nextstate = [0]*26 ## it might wrong code
            self.done = True 
