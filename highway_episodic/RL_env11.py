...
class rlEnv():
    ...
    def __toward_biggest_empty_space_reward(self,id):
        ...
        return np.exp(-dist_to_biggest_empty_space)
    ...
    def __LaneChange_SD_negative_reward(self,action): # SD_LC 침범 안하면 reward 0, SD_LC 침범한 거리 만큼 negative reward , 앞뒤 차량 SD_LC 침범시 reward -50
        ...
        LEFT_FOLLOWER =[]
        RIGHT_FOLLOWER=[]
        traci.vehicle.subscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 50.0, [tc.VAR_POSITION])
        # print(id,' subscribeContext')
        for v in traci.vehicle.getContextSubscriptionResults(str(id)):
            Left_follower_id = self.__find_followers(id, v,LEFT_FOLLOWER,RIGHT_FOLLOWER)[0]
            Right_follower_id = self.__find_followers(id, v,LEFT_FOLLOWER,RIGHT_FOLLOWER)[1]
        traci.vehicle.unsubscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 50.0)

        if action == 3: #왼쪽 차로로 이동
            if len(traci.vehicle.getLeftLeaders(id)) !=0 and Left_follower_id is not None: # 왼쪽 차로 앞 뒤 차량이 50m 안에 있다는 가정.
                ...
                if((traci.vehicle.getPosition(Left_leader_id[0][0])[0]-ego_x_pos) >= front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Left_follower_id)[0])>= back_SD_LC):
                    ...
                    return int((traci.vehicle.getPosition(Left_leader_id[0][0])[0]-ego_x_pos) - front_SD_LC )
                elif((traci.vehicle.getPosition(Left_leader_id[0][0])[0]-ego_x_pos) >= front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Left_follower_id)[0]) < back_SD_LC):
                    print('LaneChange_SD_negative_reward : left rear   ',(ego_x_pos -traci.vehicle.getPosition(Left_follower_id)[0]) - back_SD_LC)
                    return int((ego_x_pos -traci.vehicle.getPosition(Left_follower_id)[0]) - back_SD_LC)
                else:
                    print('LaneChange_SD_negative_reward : left forward and rear    ',-50)
                    return-50
                

        elif action == 4: #오른쪽 차로로 이동
            if len(traci.vehicle.getRightLeaders(id))!=0 and Right_follower_id is not None: # 오른쪽 차로 앞 뒤 차량이 50m 안에 있다는 가정.
                ...
                if((traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) >=front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0])>=back_SD_LC):
                    ...
                    return int((traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) - front_SD_LC)
                elif((traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) >=front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0])<back_SD_LC):
                    print('LaneChange_SD_negative_reward : rear     ',(ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0]) - back_SD_LC)
                    return int((ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0]) - back_SD_LC)
                else:
    ...
    def __reward(self, step_num, action):
        ...
        if('ego' in vehs):
            ...
            # if (traci.vehicle.getAngle('ego') >95 or traci.vehicle.getAngle('ego') <85) and traci.vehicle.getLaneIndex('ego')==1: # 수정 필요 -> 차로 변경 중에는 q-table 업데이트 안됨

            #     reward += self.__LaneChange_SD_negative_reward(action)
        ...
        if(rear_clearance>50 and self.__ego_vehicle_LC_start()): #후방 차량 충돌 판단 불가능 범위에서 불필요한 차로 변경
            reward -= 20
        return reward
