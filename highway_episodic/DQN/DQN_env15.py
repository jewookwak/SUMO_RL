...
class rlEnv():
    ...
    def __ego_a_desire_LQR(self, v_c,v_t,c_d,c,a_forward,a_rear): # desire acceleration by ACC, v_c : ego_vehicle's velocity, v_t : target velocity, c_d : desire clearance, c = actual clearance
        a_cur = traci.vehicle.getAcceleration('ego')
        tau = 2.27 #0.74 
        a_target = (a_forward+a_rear)/2
        clearance_error = (c_d -c)
        velocity_error = (v_t-v_c)
        ...
        Q= [[1,0],[0,1]]
        R =1
        K, S, E = ct.lqr(A, B_long1, Q, R)
        a_feedback = -K*x_long
        print('a_feedback: ',a_feedback)
    ...
    def __set_a(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if(a_cur+jerk*dt<a_des):
            ...
            traci.vehicle.setAcceleration(id,a_des,1)
            return a_des
    def __set_a2(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        jerk = 5#3
        dt = self.step_length
        a_cur = traci.vehicle.getAcceleration(id)
        if(a_cur+jerk*dt<a_des):            
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if('ego' in vehs and 'car.forward' in vehs):  
            ...
            if action == 0: 
                ...
                a_rear = traci.vehicle.getAcceleration(left_near_vehicles[1][1])
                print('left_near_vehicles[1][1]: ',left_near_vehicles[1][1])
                a_forward =traci.vehicle.getAcceleration(left_near_vehicles[2][1])
                print('left_near_vehicles[2][1]: ',left_near_vehicles[2][1])
                ...
                self.__set_ego_a('ego',self.__ego_a_desire_LQR(v_ego,v_t,1,s1,a_forward,a_rear))
    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):            
            ...
            if action == 5:
                ...
                x_next = ego_x+ego_v*self.step_length+0.5*ego_a*self.step_length**2 #등가속도 거리 공식
                y_next = ego_y+ego_v_y_desire*self.step_length
