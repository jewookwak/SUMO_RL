...
class rlEnv():
    ...
    def reset(self):
        ...
        ego_rand = random.uniform(98-15,98+15) ########### RL 검증
        forward_rand = random.uniform(160,170)
    ...
    def save_csv(self):
        ...
        col_reward = ['time','r1','r2','r2_1','r3','r4','r5','r5_1','r7','r8','r9','total']
...
########################################################
    ...
    def __reward(self, step_num, action):
        ...
        w2_1 = 0.01 #SD_LK not satisfied negative reward
        w3 = 0.005 # distance to biggest empty space positive reward
        w4 = 0.1*5 # LK continue if rear accel is far away positive reward or if rear accel is closer give negative reward
        w5 =w5_1= 0.5 # LC succeed positive reward
        w6 = 0.01 # over LC negative reward
        w7 = 0.01 # to target lane positive reward
        w8 = 0.5 # no collision until rear vehicle pass positive reward
        w9 = 0.1 # once LC start keep last step action positive reward
        reward = 0
        vehs = traci.vehicle.getIDList()
        ###negative rewards            
        #collision
            ...
            if('ego' in vehs and 'accel.rear' in vehs):           
                ...
                if traci.vehicle.getLaneIndex('ego')==1:
                    if(rear_clearance<=80 and rear_clearance>=0): # 차로 변경을 위한 가장큰 빈공간의 중앙에 가까워질 수 록 보상을 준다.
                        ...
                        Reward.append(self.__toward_biggest_empty_space_reward('ego')*w3)
                    else:
                        Reward.append(0)
                        reward +=0
                    ...
                    #     reward +=1*w4
                    if(rlEnv.ego_LC_success ==False and rear_clearance<=80 and rear_clearance >=0 and traci.vehicle.getLaneIndex('ego')==1): #후방 차량 충돌 판단 가능 범위
                ...
                if (self.done and rear_clearance<=40 and rear_clearance >=-30):
                    print('check right zero value if collision else 200 or 150 or 100 or 50')
                    print('LC success reward: ',self.__chosen_space_size_reward()*w5 )
                    reward += self.__chosen_space_size_reward()*w5
                    Reward.append(self.__chosen_space_size_reward()*w5)
                else:
                    Reward.append(0)
                    reward +=0
                ...
                if(rear_clearance>40 and self.done): #후방 차량 충돌 판단 불가능 범위에서 불필요한 차로 변경
                    print('early LC negative reward')
                    print('early LC negative reward: ',-20 *w5_1)
                    reward -= 20*w5_1
                    Reward.append(-20*w5_1)  
                else:
                    Reward.append(0)
                    reward +=0    
  
                if len(rlEnv.last_action)>0 and rear_clearance<80 and (((action == 1 or action == 3) and rlEnv.Left_SD == True) or ((action == 2 or action ==  4) and rlEnv.Right_SD == True)):                    
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs): 
            ...
            if rlEnv.Target_left_car in vehs and rlEnv.Target_right_car in vehs and 'ego' in vehs:
                ...
                if traci.vehicle.getFollower(rlEnv.Target_right_car)[0] =='ego':
                    ...
                    right_follower = traci.vehicle.getFollower(rlEnv.Target_right_car)[0]
                s1 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_leader)[0])/2 - traci.vehicle.getPosition('ego')[0] #자차와 왼쪽 차로 전방 빈공간의 중점 까지 x축 방향 상대 거리
                s2 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_leader)[0])/2 - traci.vehicle.getPosition('ego')[0] #자차와 오른쪽 차로 전방 빈공간의 중점 까지 x축 방향 상대 거리
                s3 = (traci.vehicle.getPosition(rlEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_follower)[0])/2 - traci.vehicle.getPosition('ego')[0] #자차와 왼쪽 차로 후방 빈공간의 중점 까지 x축 방향 상대 거리            
                s4 = (traci.vehicle.getPosition(rlEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_follower)[0])/2 - traci.vehicle.getPosition('ego')[0] #자차와 오른쪽 차로 후방 빈공간의 중점 까지 x축 방향 상대 거리
            ...
            if action == 0: 
                ...
                if traci.vehicle.getLaneIndex('ego') == 0:
                    ...
                    if s1 >=LC_start_distance:
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,2,s1,1))
                        
                        control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,2,s1,1),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Left_action = False     
                        rlEnv.ego_control.append(control)   
                    else:
                        self.LC_Left(action)
                ...
                if traci.vehicle.getLaneIndex('ego') == 2:
                    ...
                    if s2 >= LC_start_distance:   
                        self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,2,s2,2))
                        control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,2,s2,2),self.__set_ego_a_y_desired('ego')]
                        rlEnv.Right_action = False
                        rlEnv.ego_control.append(control)
                    else:
                        self.LC_Right(action)
