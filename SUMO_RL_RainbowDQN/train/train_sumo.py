...
from sumo_env_total_random_with_normalized_new_states_based_on_paper_preLC_fix_new_last_SD_visualization_all_actions import rlEnv
...
def save_loss(episode):
    name = str(Path(__file__).name)
    file_path = 'logs/results/'+'/'+name+'/'+str(episode)
    os.makedirs(file_path, exist_ok=True)
    os.chdir(file_path)
...
#     path = "/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/result/"
#     os.chdir(path)
#     now = datetime.now()
...
def rl_run(sumoBinary, episodenum,net, sumocfg,step_length, veh):  
    ...
    for episode in range(1,episodenum):
        ...
        while (env.done) == False:
            ...
            if env.done:
                ...
                if score_avg >max_score_avg and len(agent.memory) >= agent.train_start:
                    agent.model.save_weights("save_weights/"+name+"/best_model", save_format="tf")
                    print('max_score_avg: ',max_score_avg)
                    max_score_avg =max([max_score_avg,score_avg])
                if episode == episodenum-1 or (episode)%50 ==0:
                    agent.model.save_weights("save_weights/"+name+"/model_"+str(episode), save_format="tf")
                    # with open("/home/jewoo/Desktop/highway_episodic/DQN/sav e_model20/memory/6/replaybuffer"+str(episode), "wb") as file:
        ...
        if ((episode)%10) == 0 and not episode ==0:
            ...
            # pylab.ylabel("score")
            # pylab.title('Reward')
            # pylab.savefig("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_graph/reward.png")
            ...
            # pylab.savefig("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_graph/collision.png")
            ...
            # pylab.savefig("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_graph/LC_success_spaces.png")
            # plt.close()
            ...
            # pylab.savefig("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_graph/LC_success_spaces_size.png")
            # plt.close()
...
class DQN(tf.keras.Model):
    def __init__(self, action_size,state_size,batch_size,sequence_length):
        ...
        self.lstm2 = LSTM(64, batch_input_shape=(batch_size,sequence_length,3),return_sequences=False)
        self.lstm3 = LSTM(64, batch_input_shape=(batch_size,sequence_length,3),return_sequences=False) #spaces lstm
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        # self.model.load_weights("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_model/model_1000")
        # self.target_model.load_weights("/home/jewoo/Desktop/highway_episodic/DQN/RL_validation/save_model/best_model")
...
if __name__ == "__main__":
    ...
    sumocfg = "envs/config/highway_episodic.sumocfg"
    veh = "ego"
    options = get_options()
    if options.nogui:
