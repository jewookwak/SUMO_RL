...
class rlEnv():
    ...
    def state(self,id):
        ...
        if id:
            ...
            if traci.vehicle.getLeader(id) is not None:
                ...
                if forward_clearance>100:
                    forward_clearance =100
            else:
                forward_clearance =100
            if type(traci.vehicle.getFollower(id)) is not None and traci.vehicle.getFollower(id)[0] != '':
                ...
                if rear_clearance >100:
                    rear_clearance = 100
            else:
                rear_clearance =100
                
            states.append(round(forward_clearance))
            states.append(round(rear_clearance))
    ...
    def __chosen_space_size_reward(self,id):
        ...
        for i in range(4):
            if empty_space_dictionary_sorted_by_space[i][1][1]> traci.vehicle.getPosition('ego')[0] and empty_space_dictionary_sorted_by_space[i][1][0] <traci.vehicle.getPosition('ego')[0] :
    ...
    def __reward(self, step_num):
        ...
        if (self.__ego_vehicle_LC_success()):
            reward += self.__chosen_space_size_reward('ego')
        reward -= step_num * 0.01
        return reward
