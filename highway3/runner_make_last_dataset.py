...
# params = 'C:/Users/jewoo/OneDrive/바탕 화면/SUMO/highway3/log_data/new_data/6/parameters.csv'
...
def generate_routefile():
    ...
    with open("highway3.rou.xml", "w") as routes:
        ...
        <flow id="car1" departPos="free" begin="0.50" from="E4" to="E8" end="50.00" number="20" departSpeed="max" type="car"/>
        <flow id="car2" departPos="free" begin="3.50" from="E7" to="E6" end="50.00" number="20" departSpeed="max" type="car" color="0,1,0"/>
        <flow id="car3" departPos="free" begin="3.50" from="E7" to="E8" end="50.00" number="20" departSpeed="max" type="car" color="0,1,0"/>
        
        <flow id="truck0" departPos="free" begin="6.50" from="E4" to="E6" end="50.00" number="5" departSpeed="max" type="truck" color="0,0,1"/>  
        <flow id="truck1" departPos="free" begin="9.00" from="E7" to="E8" end="50.00" number="5" departSpeed="max" type="truck" color="0,0,1"/>      
        
        <flow id="accel0"  departPos="free" begin="17.00" from="E4" to="E6" end="50.00" number="5" type="accel_truck" color="1,0,0"/>
        <flow id="accel1"  departPos="free" begin="18.50" from="E7" to="E8" end="50.00" number="5" type="accel_truck" color="1,0,0"/>""", file=routes)
...
def save_csv():
    ...
    os.chdir('C:/Users/jewoo/OneDrive/바탕 화면/SUMO/highway3/log_data/last_dataset/10')
        ...
        for veh_id in vehs:
            ...
            if traci.vehicle.getRoadID(veh_id) == 'E5': 
                ...
                if ID[0][0] == 'a' and ID[0][-1]=='0' and not (len(accel0[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - accel0[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 'a' and ID[0][-1]=='1'and not (len(accel1[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - accel1[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 't' and ID[0][-1]=='0'and not (len(truck0[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - truck0[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 't' and ID[0][-1]=='1'and not (len(truck1[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - truck1[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 'c' and ID[0][-1]=='0'and not (len(car0[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - car0[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 'c' and ID[0][-1]=='1'and not (len(car1[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - car1[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 'c' and ID[0][-1]=='2'and not (len(car2[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - car2[int(ID[1])][-1][4])/step_length
                if ID[0][0] == 'c' and ID[0][-1]=='3'and not (len(car3[int(ID[1])])==0):
                    acceleration_x = (traci.vehicle.getSpeed(veh_id) - car3[int(ID[1])][-1][4])/step_length
            ...
            if (traci.vehicle.getRoadID(veh_id) == 'E5' and veh_id[0] == 't') or (traci.vehicle.getRoadID(veh_id) == 'E6' and veh_id[0] == 't'):
                ...
                if traci.vehicle.getLeader(veh_id) is not None:
                    ...
                    if(int(ID[0][-1])==0):
                        ...
                        elif(x_forward<random_clearance[int(veh_id[7:])+10]):
                    ...
                    if(int(ID[0][-1])==1):
                        ...
                        elif(x_forward<random_clearance[int(veh_id[7:])+15]):
