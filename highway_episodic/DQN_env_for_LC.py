...
class rlEnv():
    ...
    def __ego_vehicle_LC_completed(self):
        ...
        if 'ego' in vehs:
            if rlEnv.ego_LC_start == True:
                if (traci.vehicle.getLaneIndex('ego')== 2 or traci.vehicle.getLaneIndex('ego')== 0) and traci.vehicle.getAngle('ego') >= 89.9 and traci.vehicle.getAngle('ego') <= 90.1 and traci.vehicle.getLateralLanePosition('ego')==0:                    
    ...
    def __set_a_possible(self,id,a_y_des):
        jerk = 5
        dt = self.step_length
        if(len(rlEnv.ego)!=0):
            v_y_cur = (traci.vehicle.getPosition(id)[1] - rlEnv.ego[-1][3])/dt
    ...
    def __set_v_possible(self,id,v_des):
        ...
        print("v_y_cur-a_y_cur*dt-1/2*jerk*dt**2 : ",v_y_cur-a_y_cur*dt-1/2*jerk*dt**2)
        # a_des = (v_des - v_y_cur)/dt
        ...
        #     if (v_y_cur+a_y_cur*dt+1/2*jerk*dt**2<v_des):
        #         return v_y_cur+a_y_cur*dt+1/2*jerk*dt**2
        #     elif v_y_cur+v_y_cur+a_y_cur*dt+1/2*jerk*dt**2 >= v_des:
        #         return v_des
        # else:#감속하는 경우 (v_des = -1)
        #     if(v_y_cur+a_y_cur*dt-1/2*jerk*dt**2 > v_des):
        #         return v_y_cur+a_y_cur*dt-1/2*jerk*dt**2
        #     elif v_y_cur+a_y_cur*dt-1/2*jerk*dt**2 <= v_des:
        #         return v_des
    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):   
            ...
            if action == 5:
                ...
                ego_a_y_desire = self.__set_a_possible('ego',0.5)
    ...
    def step4(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):   
            ...
            if action == 6:
                ...
                ego_a_y = self.__get_a_y('ego')
                # ego_v_y_desire = self.__set_v_possible('ego',ego_v_y_desire)
                ego_a_y_desire = ego_a_y_desire = self.__set_a_possible('ego',-0.5)
                ego_v_y_next = ego_v_y +ego_a_y_desire*self.step_length
