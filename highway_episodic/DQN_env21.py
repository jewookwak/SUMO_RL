...
class rlEnv():
    ...
    def __ego_vehicle_LC_completed(self):
        ...
        if 'ego' in vehs:
            if rlEnv.ego_LC_start == True:
                if (traci.vehicle.getLaneIndex('ego')== 2 or traci.vehicle.getLaneIndex('ego')== 0) and traci.vehicle.getAngle('ego') >= 89.999 and traci.vehicle.getAngle('ego') <= 90.001 and np.abs(traci.vehicle.getLateralLanePosition('ego'))<=0.01:                    
        ...
        if 'ego' in vehs:
            ...
            if lane == 0 and LLP <0:
                if action == 5:
                    ...
                    right_target_space = [(right_near_vehicles[1][0]+right_near_vehicles[0][0])/2,-4.8-3.2]
                    print("Wrong action!!")
                    return right_target_space
            ...
            if lane == 1 and LLP<0:
                if action == 5:
                    ...
                    if len(left_vehicles)<=1:
                        ...
                        left_near_vehicles.sort()
                        left_target_space = [(left_near_vehicles[1][0]+left_near_vehicles[0][0])/2,-4.8]
                        print('left_target_space: ',left_target_space )
                        print(LLP)
                    return left_target_space
                elif action == 6:
                    for v_id in traci.vehicle.getContextSubscriptionResults(str(id)):
            ...
            if lane == 1 and LLP==0:
                if action == 5:
                    ...
                    for i in range(2):
                        left_near_vehicles.append([left_vehicles[:3][i][1],left_vehicles[:3][i][2]])# 1: left vehicle position 2: left vehicle id
                    left_near_vehicles.sort()
                    left_target_space = [(left_near_vehicles[1][0]+left_near_vehicles[0][0])/2,-4.8+3.2]
                    print('left_target_space: ',left_target_space)
                    print(LLP)
                    return left_target_space
                elif action == 6:
    ...
    def __set_ego_v_movetoXY(self,id,v_des): # v_des에 따라 a_des을 결정하고 jerk 값이 고려된  속도를 시스템에 넣음.
        ...
        if (v_des - v_cur)>= a_max:
            ...
            a_des = a_min
        # a = set_a(id,a_des)
        self.__set_ego_a_movetoXY(id,a_des)
        return a_des
    ...
    def __toward_biggest_empty_space_reward(self,id):
        ...
        if empty_space_dictionary_sorted_by_space[0][0] == 'L0':
            lateral_distance_error = np.abs(-1.6 -traci.vehicle.getPosition('ego')[1]) #1차로 중앙으로 부터 자차까지 거리
        elif empty_space_dictionary_sorted_by_space[0][0] == 'R0':
    ...
    def __reward(self, step_num, action):
        ...
        if('ego' in vehs and 'accel.rear' in vehs):           
            ...
            if traci.vehicle.getLaneIndex('ego')==1:
                if(rear_clearance<=70 and rear_clearance>=0): # 차로 변경을 위한 가장큰 빈공간의 중앙에 가까워질 수 록 보상을 준다.
                ...
                if rear_clearance>70 and (action ==0 or action ==1 or action ==2):
                    ...
                    reward +=1*w4
                if(rlEnv.ego_LC_success ==False and rear_clearance<=70 and traci.vehicle.getLaneIndex('ego')==1): #후방 차량 충돌 판단 가능 범위
                    # print('Danger LK continue reward: ' ,-1*w4)
                    reward -= 1.1**(10-rear_clearance)*w4
        
        
            if (self.__ego_vehicle_LC_completed() and rear_clearance<=70 and rear_clearance >=0):
                # print('LC success reward: ',self.__chosen_space_size_reward('ego')*w5 )
                reward += self.__chosen_space_size_reward('ego')*w5
            # vehs = traci.vehicle.getIDList()
            
            
            if(rear_clearance>70 and self.__ego_vehicle_LC_completed()): #후방 차량 충돌 판단 불가능 범위에서 불필요한 차로 변경
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs): 
            ...
            if action == 0: 
                if Anti_RVC_on == True: #LK_mode
                    ...
                    control = [traci.simulation.getTime(),self.__set_ego_v_movetoXY('ego',speed),0]
                else:
                    # self.__set_ego_v('ego',ego_v_last)
                    control = [traci.simulation.getTime(),self.__set_ego_v_movetoXY('ego',ego_v_last),0]
            elif action == 1:
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs):
            ...
            nextstate = [0]*24 ## it might wrong code
    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                # goal_x,goal_y = self.__LC_goal_space_posistion(action)
                # goal_x_rel = goal_x - ego_x
                # goal_y_rel = goal_y - ego_y
                if len(rlEnv.last_action) != 0:
                    if rlEnv.last_action[-1][1] != 5:
                        ...
                        rlEnv.initail_v_y =ego_v_y
                        rlEnv.initial_a_y = ego_a_y
                    else:
                ...
                # print(t)
                # print(rlEnv.last_action[-1][1])
                a_r = 1.4 #max acceleration up to rollover.
                target_y = self.__LC_goal_space_posistion(action)[1]
                d_lateral = target_y -rlEnv.initial_y 
                print('rlEnv.initial_y: ',rlEnv.initial_y )
                print('d_lateral: ',d_lateral)
                t_des = np.sqrt(2*d_lateral*np.pi/a_r)
                print(t_des)
                ...
                if(t_des ==0):
                    ...
                    if rlEnv.initial_a_y !=0:
                        A = np.abs(1.4 - ego_a_y)                        
                    else:
                        A = 1.4
                    omega = 2*np.pi/t_des
                print('t: ',t)          
                ego_a_y_des= A*np.sin(omega*t)
                d_lateral = target_y -rlEnv.initial_y 
                # if rlEnv.initial_y >-4.8:
                #     print("second LC sin")
                #     print("value: ",rlEnv.initail_v_y**2/4-1.4/np.pi*d_lateral)
                #     t_des = (rlEnv.initail_v_y/2+np.sqrt(rlEnv.initail_v_y**2/4-1.4/np.pi*d_lateral))*2*np.pi/1.4
                #     A=1.4
                #     omega = 2*np.pi/t_des
                #     ego_a_y_des = A*np.sin(omega*(t-t_des/2))
                ego_a_y_desire = self.__set_a_y_possible('ego',ego_a_y_des)    
    ...
    def step4(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 6:
                ...
                a_r = 1.4 #max acceleration up to rollover.
                target_y = self.__LC_goal_space_posistion(action)[1]
                d_lateral = np.abs(target_y -rlEnv.initial_y)
                t_des = np.sqrt(2*d_lateral*np.pi/a_r)
                ...
                if(t_des ==0):
                    ...
                    omega = 2*np.pi/t_des          
                ego_a_y_des= -A*np.sin(omega*t) #음수(오른쪽 LC는 왼쪽LC의 반대로 계산)
