...
params = 'C:/Users/jewoo/OneDrive/바탕 화면/SUMO/highway3/log_data/last_dataset/1/parameters.csv'
...
def generate_routefile():
    ...
    with open("highway3.rou.xml", "w") as routes:
        print("""<routes>
        <vType accel="3" decel="4.5" departSpeed = "22" id="car" length="5" maxSpeed="22.22" minGap="2.0" sigma="1" lcSpeedGain ="0"/>
        ...
        <flow id="car0" departPos="free" begin="0.00" from="E4" to="E6" end="25.00" number="5" departSpeed="max" type="car" color="0,1,0"/>
        <flow id="car1" departPos="free" begin="0.00" from="E4" to="E8" end="25.00" number="5" departSpeed="max" type="car" color="0,1,0"/>
        <flow id="car2" departPos="free" begin="0.00" from="E7" to="E6" end="25.00" number="5" departSpeed="max" type="car" color="0,1,0"/>
        <flow id="car3" departPos="free" begin="0.00" from="E7" to="E8" end="25.00" number="5" departSpeed="max" type="car" color="0,1,0"/>
...
def save_csv():
    ...
    os.chdir('C:/Users/jewoo/OneDrive/바탕 화면/SUMO/highway3/log_data/last_anti_rear_collision_dataset/1/checking2/20')
...
def a_desire_with_speed_limit(v_c,v_p,c_d,c,speed_limit): #v_c : 자차 속도, v_p : 앞 차속도, c_d : 요구 clearance ( c_d = c0+tau*v_preceding), c : clearance, speed_limit 제한 속도
    v_define = min(speed_limit,v_p) # 앞차 속도와 규정 속도 중 작은 것을 따름.
    
    if(v_c > speed_limit):
        a_d = - K2(v_c)*(speed_limit-v_c)
    else:
        a_d = -K1(v_c)*(c_d-c) - K2(v_c)*(v_define-v_c) #앛
    if(a_d > a_max(v_c)):
        return a_max(v_c)
    elif(a_d <= a_max(v_c) and a_d >=a_min(v_c)):
        return a_d
...
def set_a(id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
    jerk = 3
    dt = step_length
...
def set_v(id,v_des): # v_des에 따라 a_des을 결정 set_a 에 그 값을 넘겨주어서 가속도 결정함.
    dt = step_length
    Kp =0.9 # p gain
    v_cur = traci.vehicle.getSpeed(id)
...
def setvehiclestate2(id, goal_v_x,min_a_x, max_a_x, goal_j_x, desire_a_x): # 원하는 가속도 값을 갖는 주행을 할 수 있도록 하는 함수
    dt = step_length
    if id[0] == 'a':
        if id[5] == '0':
            last_v_x = accel_truck0_buffer[int(id[-1])][0][4]
    ...
    k=goal_j_x*dt+0.01 #jerk*dt 보다 커야함.
    a_cur = traci.vehicle.getAcceleration(id)
    if(a_cur<desire_a_x-k):
        traci.vehicle.setAcceleration(id,a_cur+goal_j_x*dt,1)
        accel = a_cur+goal_j_x*dt
    elif(a_cur>desire_a_x+k):
        traci.vehicle.setAcceleration(id,a_cur-goal_j_x*dt,1)
        accel = a_cur-goal_j_x*dt
    else:
        traci.vehicle.setAcceleration(id,desire_a_x,1)
        accel = desire_a_x
...
def run():  
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for veh_id in vehs:
            ...
            if (traci.vehicle.getRoadID(veh_id) == 'E4' and veh_id[0] == 'a') or (traci.vehicle.getRoadID(veh_id) == 'E7' and veh_id[0] == 'a') or (traci.vehicle.getRoadID(veh_id) == 'E4' and veh_id[0] == 't') or (traci.vehicle.getRoadID(veh_id) == 'E7' and veh_id[0] == 't'):
                traci.vehicle.setLaneChangeMode(veh_id,0b011001010101)
                ID = veh_id.split('.')
                if(int(ID[1])%2 ==1):
                    traci.vehicle.changeLaneRelative(veh_id,1,5)
            if traci.vehicle.getRoadID(veh_id) == 'E4' and veh_id[0:4] == 'car0':
                traci.vehicle.setLaneChangeMode(veh_id,0b011001010101)
                ID = veh_id.split('.')
                if(int(ID[1])%2 ==1):
                    traci.vehicle.changeLaneRelative(veh_id,1,5)
            ...
            if traci.vehicle.getRoadID(veh_id) == 'E5' and veh_id[0] == 'a':
                ...
                if(int(ID[0][-1])==1):
                    setvehiclestate(veh_id,random_speed[int(veh_id[7:])+5],random_min_a[int(veh_id[7:])+5],random_max_a_x[int(veh_id[7:])+5],random_jerk[int(veh_id[7:])+5])
