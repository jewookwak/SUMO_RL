...
def save_csv():
    ...
    os.chdir('C:/Users/jewoo/OneDrive/바탕 화면/SUMO/highway3/log_data/last_anti_rear_collision_dataset/1/checking2/21')
...
def setvehiclestate2(id, goal_v_x,min_a_x, max_a_x, goal_j_x, desire_a_x): # 원하는 가속도 값을 갖는 주행을 할 수 있도록 하는 함수
    ...
    if(a_cur<desire_a_x-k):
        traci.vehicle.setAcceleration(id,a_cur+goal_j_x*dt,20)
        accel = a_cur+goal_j_x*dt
    elif(a_cur>desire_a_x+k):
        traci.vehicle.setAcceleration(id,a_cur-goal_j_x*dt,20)
        accel = a_cur-goal_j_x*dt
    else:
        traci.vehicle.setAcceleration(id,desire_a_x,20)
        accel = desire_a_x
