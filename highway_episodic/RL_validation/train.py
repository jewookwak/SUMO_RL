...
def rl_run(sumoBinary, episodenum,net, sumocfg,step_length, veh):  
    ...
    for episode in range(1,episodenum):
        ...
        action = 0 #first action is 0 -> arbitrary selection
        if action == 5:
            action = 7
        while (env.done) == False:  
            ...
            if env.done:
                ...
                if episode >= 2 and (episode+1)%20 == 0:
                    agent.update_target_model()
                # 에피소드마다 학습 결과 출력
                ...
                # 이동 평균이 70 이상일 때 종료
                if score_avg >= 150:
                    agent.model.save_weights("/home/mds/Desktop/highway_episodic/DQN/RL_validation/save_model/best_model", save_format="tf")
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        # self.model.load_weights("/home/mds/Desktop/highway_episodic/DQN/RL_validation/save_model/best_model")
    ...
    def train_model(self,episode):
        if self.epsilon > self.epsilon_min and (int((np.round(traci.simulation.getTime(),2)*100)%50) == 0) and (episode+1)%10 == 0:
