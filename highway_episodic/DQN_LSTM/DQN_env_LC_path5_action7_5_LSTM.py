...
class rlEnv():
    ...
    def save_csv(self):
        ...
        col_reward = ['time','r1','r2','r2_1','r3','r4','r5','r5_1','r7','r8','total']
...
########################################################
    ...
    def __LaneChange_SD_negative_reward(self,action): # SD_LC 침범 안하면 reward 0, SD_LC 침범한 거리 만큼 negative reward , 앞뒤 차량 SD_LC 침범시 reward -50
        ...
        for v in traci.vehicle.getContextSubscriptionResults(str(id)):
            ...
            Right_follower_id = self.__find_followers(id, v,LEFT_FOLLOWER,RIGHT_FOLLOWER)[1]
        traci.vehicle.unsubscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 200.0)
        if traci.vehicle.getLaneIndex(id) == (0 or 2):
            front_relative_velocity_term = max( [traci.vehicle.getSpeed(id) - traci.vehicle.getSpeed(Leader_id) ,0] )*time_gap_LC_1
            front_minimum_clearance_trem = max([traci.vehicle.getSpeed(id)*time_gap_LC_2,c_LC])
            front_SD_LC = front_relative_velocity_term +front_minimum_clearance_trem
            back_relative_velocity_term = max( [traci.vehicle.getSpeed(follower_id)-traci.vehicle.getSpeed(id),0] )*time_gap_LC_1
            ...
            if len(traci.vehicle.getRightLeaders(id))!=0 and Right_follower_id is not None: # 오른쪽 차로 앞 뒤 차량이 100m 안에 있다는 가정.
                ...
                if((traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) >=front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0])>=back_SD_LC):
                    ...
                    # print('LaneChange_SD_negative_reward : forward     ',(traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) - front_SD_LC)
                    return (traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) - front_SD_LC
                elif((traci.vehicle.getPosition(Right_leader_id[0][0])[0]-ego_x_pos) >=front_SD_LC  and (ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0])<back_SD_LC):
                    # print('LaneChange_SD_negative_reward : rear     ',(ego_x_pos -traci.vehicle.getPosition(Right_follower_id)[0]) - back_SD_LC)
    ...
    def __reward(self, step_num, action):
        ...
        w2_1 = 0.002 #SD_LK not satisfied negative reward
        ...
        if action == 7 and Lane == 1 and np.abs(LLP)<=0.56 and (rear_TTC<2.4 or (rlEnv.Left_SD == False and rlEnv.Right_SD == False)):
            reward=0 #reward 0
            Reward.append(0)
            Reward.append(0)
            Reward.append(0)
            ...
            if('ego' in vehs and 'accel.rear' in vehs):           
                ...
                if (self.__ego_vehicle_LC_completed() and rear_clearance<=80 and rear_clearance >=-30):
                    # print('LC success reward: ',self.__chosen_space_size_reward('ego')*w5 )
                    reward += self.__chosen_space_size_reward('ego')*w5
                    Reward.append(self.__chosen_space_size_reward('ego')*w5)
                else:
                    Reward.append(0)
                    reward +=0
                ...
                if(rear_clearance>80 and self.__ego_vehicle_LC_completed()): #후방 차량 충돌 판단 불가능 범위에서 불필요한 차로 변경
                    ...
                    Reward.append(0)
                    reward +=0    
                # print('rear_clearance: ',rear_clearance<80)
                # print('rlEnv.Left_SD: ',rlEnv.Left_SD)
                # print('rlEnv.Right_SD: ',rlEnv.Right_SD)
                if len(rlEnv.last_action)>0 and rear_clearance<80 and ((action == 5 and rlEnv.Left_SD == True) or (action == 6 and rlEnv.Right_SD == True)):                    
                    if rlEnv.last_action[-1][1] == 5 and action == 5:
                        if Lane == 1 and LLP >0:
                            reward+= LLP*w7
                            ...
                            reward+=0
                            Reward.append(0)
                    elif rlEnv.last_action[-1][1] == 6 and action == 6:
                        if Lane == 1 and LLP <0:
                            reward+= -LLP*w7
                            Reward.append(-LLP*w7)
                ...
                # Lane = traci.vehicle.getLaneIndex('ego')
                # if len(rlEnv.ego)>0:
                #     if(Lane == 0 and action==6):
                #         if LLP >=0 and rlEnv.ego[-1][7]<=-1.2:
                #             reward -= 2**(1-LLP)*w6
                #             Reward.append(-2**(1-LLP)*w6)
                #             print("LLP*w6: ",LLP*w6)
                #         else:
                #             Reward.append(0)
                #         if LLP<0:
                #             reward += LLP*w6_1
                #             print("LLP*w6: ",LLP*w6_1)
                #             Reward.append(LLP*w6_1)
                ...
                #         Reward.append(0)
                #         Reward.append(0)
                #     if(Lane == 2 and action==5):
                #         if LLP<=0 and rlEnv.ego[-1][7]>=1.2:
                #             reward -=2**(1+LLP)*w6_2
                #             print("LLP*w6: ",LLP*w6_2)
                #             Reward.append(-2**(1+LLP)*w6_2)
                #         else:
                #             Reward.append(0)
                #         if LLP>0:
                #             reward -=LLP*w6_3
                #             print("LLP*w6: ",LLP*w6_3)
                #             Reward.append(-LLP*w6_3)
                #         else:
                #             Reward.append(0)
                #     else:
                #         Reward.append(0)
    ...
    def LC_cubic_polynomial_path(self,x_initial,y_initial,x_final,y_final,rear_v_id,forward_v_id,theta_i,action):
        ...
        # pylab.xlabel("x")
        # pylab.ylabel("y")
        # pylab.savefig("/home/mds/Desktop/highway_episodic/DQN/path/graph.png")
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs): 
            ...
            if action == 0: 
                ...
                self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,1,s1,1))
                
                control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,1,s1,1),self.__set_ego_a_y_desired('ego')]
                ...
                self.__set_ego_a_movetoXY('ego',self.__ego_a_desire(v_ego,v_t,1,s2,1))
                control = [traci.simulation.getTime(),self.__ego_a_desire(v_ego,v_t,1,s2,1),self.__set_ego_a_y_desired('ego')]
                ...
                self.__set_ego_a_movetoXY('ego',self.__ego_a_desire2(v_ego,v_t,1,s3,1))
                control = [traci.simulation.getTime(),self.__ego_a_desire2(v_ego,v_t,1,s3,1),self.__set_ego_a_y_desired('ego')]
            elif action == 4: #s4 <0
                ...
                self.__set_ego_a_movetoXY('ego',self.__ego_a_desire2(v_ego,v_t,1,s4,1))
                control = [traci.simulation.getTime(),self.__ego_a_desire2(v_ego,v_t,1,s4,1),self.__set_ego_a_y_desired('ego')]
            elif action == 7:
                control = [traci.simulation.getTime(),self.__set_ego_v_movetoXY('ego',ego_v_last)[0],self.__set_ego_v_movetoXY('ego',ego_v_last)[1]]
    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                if path_done == True and lane ==1 and LLP <= 0 and LLP >= -0.4 and traci.vehicle.getAngle('ego')>=89.9:
                    print("---------------------------------------LC_succeed")
                ...
                if len(rlEnv.last_action) != 0:
                    if rlEnv.last_action[-1][1] != 5:
                        ...
                        if LC_succeed:       
                            print("rear_v_id: ",rear_v_id)
                            print("forward_v_id: ",forward_v_id)                
                            rlEnv.coefficient,rlEnv.final_x,rlEnv.final_y =self.LC_cubic_polynomial_path(rlEnv.initial_x,rlEnv.initial_y,target_x,target_y,rear_v_id,forward_v_id,rlEnv.theta_i,action)
                    ...
                    rlEnv.initial_target_y = target_y
                    rlEnv.theta_i = (90-traci.vehicle.getAngle('ego'))*np.pi/180
                    rlEnv.coefficient,_,_ =self.LC_cubic_polynomial_path(rlEnv.initial_x,rlEnv.initial_y,target_x,target_y,rear_v_id,forward_v_id,rlEnv.theta_i,action)
                # print("last action : ",rlEnv.last_action[-1][1])
        ...
        self.ego_collision_happened()
        rlEnv.last_action.append([traci.simulation.getTime(),5])
    ...
    def step4(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 6:
                ...
                if path_done == True and lane == 1 and LLP <=0.4 and LLP>=0 and traci.vehicle.getAngle('ego')<=90.1:
                    print("---------------------------------------LC_succeed")
                    print("LLP: ",LLP)
                    rlEnv.initial_x = ego_x
                    rlEnv.initial_y = ego_y
        ...
        self.ego_collision_happened()
        rlEnv.last_action.append([traci.simulation.getTime(),6])
        return nextstate,nextreward
