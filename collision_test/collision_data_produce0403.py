...
def generate_routefile():
    ...
    with open("highway.rou.xml", "w") as routes:
        ...
        <vType accel="2.5" decel="4.5" departSpeed = "22" emergencyDecel="9.0" id="truck" length="12" maxSpeed="33.3333" minGap="0.0" sigma="1.0" collisionMinGapFactor="0.0" tau="2.0" lcSpeedGain ="0" vClass="truck" color="0,0,1"/>
        <vType accel="2.5" decel="4.5" departSpeed = "22" id="accel_truck" length="12" maxSpeed="33.3333" minGap="0.0" sigma="1.0" collisionMinGapFactor="0.0" tau="0.0001" lcSpeedGain ="0" vClass="truck" color="1,0,0"/>
        
        <flow id="car0" departPos="free" departLane="2" begin="0.00" from="E5" to="E1" end="500.00" number="20" type="car"/>
        <flow id="car1" departPos="free" departLane="1"  begin="5.00" from="E5" to="E1" end="500.00" number="20" type="car"/>
        <flow id="car2" departPos="free" departLane="0"  begin="10.00" from="E5" to="E1" end="500.00" number="20" type="car" color="0,1,0"/>
        <flow id="car3" departPos="free" departLane="2"  begin="15.00" from="E5" to="E1" end="500.00" number="20" type="car" color="0,1,0"/>
        <flow id="truck0" departPos="free" departLane="2" begin="20.00" from="E5" to="E1" end="500.00" number="5" type="truck" color="0,0,1"/>
        <flow id="truck1" departPos="free" departLane="1" begin="25.00" from="E5" to="E1" end="500.00" number="5" type="truck" color="0,0,1"/>
        <flow id="car4" departPos="free" departLane="1" begin="30.00" from="E5" to="E1" end="500.00" number="20" type="car"/>
        <flow id="car5" departPos="free" departLane="0"  begin="35.00" from="E5" to="E1" end="500.00" number="20" type="car"/>
        <flow id="car6" departPos="free" departLane="2"  begin="40.00" from="E5" to="E1" end="500.00" number="20" type="car" color="0,1,0"/>
        <flow id="car7" departPos="free" departLane="1"  begin="45.00" from="E5" to="E1" end="500.00" number="20" type="car" color="0,1,0"/>
        <flow id="truck2" departPos="free" departLane="0" begin="60.00" from="E5" to="E1" end="500.00" number="5" type="truck" color="0,0,1"/>
        <flow id="truck3" departPos="free" departLane="2" begin="65.00" from="E5" to="E1" end="500.00" number="5" type="truck" color="0,0,1"/>
       """, file=routes)
...
def run():
    ...
    for i in range(200):
        ...
        random_acceleration.append(float(random.randint(0,300))/100)
        random_deceleration.append(float(random.randint(0,800))/100)
        random_LCduration.append(float(random.randint(300,600))/100)
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for veh_id in e1_vehicles:
            print('veh_id[0][-1]: ',veh_id[0][-1])
            if veh_id[0][-1] == 't':
                ID = veh_id.split('.')
                if int(ID[1])%2 == 0:
                    traci.vehicle.setSpeedMode(veh_id,31)  # 모든 안전 체크 활성화
                else:
                    traci.vehicle.setSpeedMode(veh_id,0)  # 모든 안전 체크 비활성화
                traci.vehicle.setMinGap(veh_id, 0.0)  # 최소 안전 거리를 0으로 설정
                traci.vehicle.setSpeed(veh_id,random_speed[int(ID[0][-1])*10+int(ID[1])])
                traci.vehicle.setAccel(veh_id,random_acceleration[int(ID[0][-1])*10+int(ID[1])])
                traci.vehicle.setDecel(veh_id,random_deceleration[int(ID[0][-1])*10+int(ID[1])]) 
            ID = veh_id.split('.')
            if ID[0] == 'car0' or ID[0] == 'car1' or ID[0] == 'car4' or ID[0] == 'car5':
                traci.vehicle.setLaneChangeMode(veh_id, 0)
        # e4_vehicles = traci.edge.getLastStepVehicleIDs('E4')
        # for veh_id in e4_vehicles:
        #     ID = veh_id.split('.')
        #     if int(ID[1])%2 == 0:
        #         traci.vehicle.changeLaneRelative(veh_id, 1, random_LCduration[int(ID[0][-1])*10+int(ID[1])])
