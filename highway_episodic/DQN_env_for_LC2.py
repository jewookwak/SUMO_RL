...
class rlEnv():
    ...
    def __ego_vehicle_LC_completed(self):
        ...
        if 'ego' in vehs:
            if rlEnv.ego_LC_start == True:
                if (traci.vehicle.getLaneIndex('ego')== 2 or traci.vehicle.getLaneIndex('ego')== 0) and traci.vehicle.getAngle('ego') >= 89.9 and traci.vehicle.getAngle('ego') <= 90.1:                    
    ...
    #     return next_states
    def __frenet(self,S,dt,duration=3):
        # s = 6 #distance to empty space
        print('S: ',S)
        print('round S: ',int(S))
        x = np.linspace(0, int(S), int(duration/dt))
        dx=[]
        dy=[]
        ds =[]
        road_width = 3.2
        y = (np.tanh(x-S/2)+1)/2*road_width
        ...
        for i in range(len(x)-1):
            ds.append((dx[i])*np.sqrt(1+(dx[i]/dy[i])**2))
        return degree,ds
    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):   
            print('LaneIndex: ',traci.vehicle.getLaneIndex('ego'))
            print('LaneID: ',traci.vehicle.getLaneID('ego'))
            print('LateralLanePosition: ',traci.vehicle.getLateralLanePosition('ego'))
            ...
            left_vehicles.sort()
            right_vehicles.sort()
            S= (left_vehicles[0][1]+left_vehicles[1][1])/2 -traci.vehicle.getPosition('ego')[0]
            ...
            if action == 5:
                ...
                y_next = ego_y+ego_v_y*self.step_length
                
                yaw,ds = self.__frenet(S,self.step_length,3)
                angle=90-yaw[rlEnv.L_cnt]
                
                if S>=0:
                    x_next = ego_x+ds[rlEnv.L_cnt]*np.tan(yaw[rlEnv.L_cnt])
                else:
                    x_next = ego_x-ds[rlEnv.L_cnt]*np.tan(yaw[rlEnv.L_cnt])
                y_next = ego_y+ds[rlEnv.L_cnt]*np.tan(yaw[rlEnv.L_cnt])
