...
class rlEnv():
    ...
    def reset(self):
        ...
        traci.vehicle.add(vehID='car.forward',routeID='route0',typeID="car",depart='now',departLane='1',departPos='130', departSpeed='22.22', arrivalLane='current', arrivalPos='max', arrivalSpeed='current', fromTaz='', toTaz='', line='', personCapacity=0, personNumber=0)
        ...
        if id:
            ...
            for v_id in traci.vehicle.getContextSubscriptionResults(str(id)):
                if(traci.vehicle.getLaneIndex(v_id)==2) and not (v_id=='ego'):
                    ...
                    left_vehicles.append([round(np.abs(traci.vehicle.getPosition('ego')[0]- traci.vehicle.getPosition(v_id)[0]),1),round(traci.vehicle.getPosition(v_id)[0],1),v_id])
                if(traci.vehicle.getLaneIndex(v_id)==0)and not (v_id=='ego') :
                    right_vehicles.append([round(np.abs(traci.vehicle.getPosition('ego')[0]- traci.vehicle.getPosition(v_id)[0]),1),round(traci.vehicle.getPosition(v_id)[0],1),v_id])
            ...
            left_near_spaces.append([round(left_near_vehicles[0][0]+(left_near_vehicles[1][0]-left_near_vehicles[0][0])/2,1),left_near_vehicles[1][0]-left_near_vehicles[0][0]])
            right_near_spaces.append([round(right_near_vehicles[0][0]+(right_near_vehicles[1][0]-right_near_vehicles[0][0])/2,1), right_near_vehicles[1][0]-right_near_vehicles[0][0]])

            ego_to_left_spaces.append(left_near_spaces[0][0]- round(traci.vehicle.getPosition('ego')[0],1))
            ego_to_right_spaces.append(right_near_spaces[0][0]-round(traci.vehicle.getPosition('ego')[0],1))
            ...
            states.append(round(forward_clearance,1))
            states.append(round(rear_clearance,1))
                        
            states.append(round(ego_to_left_spaces[0],1)) # mapping negative numbers to positive number by add 5
            
            states.append(round(ego_to_right_spaces[0],1))# mapping negative numbers to positive number by add 5
            
            states.append(round(left_near_spaces[0][1],1))
            
            states.append(round(right_near_spaces[0][1],1))
            states.append(round(ego_to_left_leader,1))
            states.append(round(ego_to_left_follower,1))
            states.append(round(ego_to_right_leader,1))
            states.append(round(ego_to_right_follower,1))

            states.append(round(rel_v_rear,1))
            states.append(round(rel_v_forward,1))
            states.append(round(rel_v_left_forward,1))
            states.append(round(rel_v_left_rear,1))
            states.append(round(rel_v_right_forward,1))
            states.append(round(rel_v_right_rear,1))
            # print('left_near_vehicles : ',left_near_vehicles)
            # print('right_near_vehicles : ',right_near_vehicles)
    ...
    def __toward_biggest_empty_space_reward(self,id):
        ...
        return round(max([-1,-(dist_to_biggest_empty_space-15)**3/75]),2)
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        if('ego' in vehs):  
            traci.vehicle.subscribeContext('ego', tc.CMD_GET_VEHICLE_VARIABLE, 30.0, [tc.VAR_POSITION])
            # print(id,' subscribeContext')
            ...
            # print('left_vehicles[0][1]: ',left_vehicles[0][1])
            # print('right_vehicles: ',right_vehicles)
            # print('right_vehicles[0][1]: ',right_vehicles[0][1])
            left_vehicle_v = traci.vehicle.getSpeed(left_vehicles[0][1])
            right_vehicle_v = traci.vehicle.getSpeed(right_vehicles[0][1])

            # if action == 0:
            #     self.__set_v('ego',ego_v_last)
            # elif action == 1: # left overtake action
            #     if left_vehicle_v < ego_v_last:
            #         self.__set_v('ego',ego_v_last +1)
            #     else:
            #         self.__set_v('ego',left_vehicle_v - ego_v_last +1)
            # elif action == 2: # right overtake action
            #     if right_vehicle_v <ego_v_last:
            #         self.__set_v('ego',ego_v_last +1)
            #     else:
            #         self.__set_v('ego',right_vehicle_v - ego_v_last +1)
            # elif action == 3: #left vehicle pass first and then ego LC
            #     if left_vehicle_v <= ego_v_last: 
            #         self.__set_v('ego',left_vehicle_v - ego_v_last -1)
            #     else:
            #         self.__set_v('ego',ego_v_last -1)
            # elif action == 4:#right vehicle pass first and then ego LC
            #     if right_vehicle_v <= ego_v_last:
            #         self.__set_v('ego',right_vehicle_v - ego_v_last -1)
            #     else:
            #         self.__set_v('ego',ego_v_last -1)
