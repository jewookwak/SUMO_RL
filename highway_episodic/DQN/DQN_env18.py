...
class rlEnv():
    ...
    def __set_ego_a_movetoXY(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if len(rlEnv.ego)!=0:
            v_y_last = rlEnv.ego[-1][5]
            a_y_last = rlEnv.ego[-1][7]
        else:
            v_y_last = 0
            a_y_last = 0
        v_y_cur = v_y_last
    ...
    def __toward_biggest_empty_space_reward(self,id):
        ...
        if dist_to_biggest_empty_space<8:
            lateral_distance_to_side_lane = np.abs(traci.vehicle.getPosition('ego')[1]+4.8)
        else:
            lateral_distance_to_side_lane=0
        return round(max([-1,-(dist_to_biggest_empty_space-15)**3/75]),3) +lateral_distance_to_side_lane
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        ...
        rlEnv.last_action.append(action)
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                if len(rlEnv.last_action) != 0:
                    ...
                    t = 0
                    rlEnv.t_LC_start = traci.simulation.getTime()
                print("last action : ",rlEnv.last_action)
                print(t)
                ego_a_y_desire = self.__set_a_y_possible('ego',1.178*np.sin(np.pi/2*t))    
                ...
                # print("y_next: ",y_next)
                # print("")
                control = [traci.simulation.getTime(),self.__ego_acc(ego_v,v_preceding,c_desire,c_front),ego_a_y_desire]
                rlEnv.ego_control.append(control)
