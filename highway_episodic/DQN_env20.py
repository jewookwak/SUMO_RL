    ...
    def step3(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                # lateral_distance = target_y - y0 +alpha
                lateral_distance=3.2+alpha
                A = (lateral_distance-(y0+4.8))/2 # 4.8 더하는 것은 처음 차량의 y = -4.8이니깐.
                ego_a_y_des= A*np.sin(2*np.pi/lateral_distance*t)
                # ego_a_y_des = 1.178*np.sin(np.pi/2*t)
                # if LLP<0 and t<=2 and ego_v_y>1:
                #     ego_a_y_des = 0.5*np.sin(2*np.pi*t)
                ego_a_y_desire = self.__set_a_y_possible('ego',ego_a_y_des)    
    ...
    def step4(self, action): # continue the simulation step but return finalstate and finalreward as result of Lanechange.
        ...
        if('ego' in vehs):  
            ...
            if action == 6:
                ...
                # lateral_distance = target_y - y0 +alpha
                lateral_distance =-3.2+alpha
                A = ((y0+4.8)-lateral_distance)/2 ##step3와 다름!! 4.8 더하는 것은 처음 차량의 y = -4.8이니깐.
                
                ego_a_y_des= -A*np.sin(2*np.pi/np.abs(lateral_distance)*t)
                # if LLP>0 and t<=2:
                #     ego_a_y_des = -1*np.sin(np.pi*t)
