# envs/sumo_origin_simple_env.py
import os
import sys
import time
...
class simpleEnv():
    ...
    rear_tau_aggresive = random.uniform(0.1, 0.5)
    ...
    def reset(self):
        ...
        #initiate values of class simpleEnv.
        simpleEnv.Rule_RL = []
        simpleEnv.last_state =[]
        simpleEnv.lane_buffer_ego = 0
        simpleEnv.ego_LC_success = False
        simpleEnv.ego_LC_start = False
        simpleEnv.ego_LC_completed = False
        simpleEnv.last_Space1=0
        simpleEnv.last_Space2=0
        simpleEnv.last_Space3=0
        simpleEnv.last_Space4=0
        simpleEnv.last_safe_Space1=0
        simpleEnv.last_safe_Space2=0
        simpleEnv.last_safe_Space3=0
        simpleEnv.last_safe_Space4=0
        simpleEnv.S=[0,0,0,0,0,0,0,0,0]
        simpleEnv.ego_at_Target=[0,0,0,0]
        simpleEnv.integral_clearance_e=[0,0,0,0,0]
        simpleEnv.integral_velocity_e=[0,0,0,0,0]
        simpleEnv.pre_error_clearance = [0,0,0,0,0]
        simpleEnv.pre_error_velocity = [0,0,0,0,0]
        simpleEnv.ego_control=[]
        simpleEnv.ego=[]
        simpleEnv.left0=[]
        simpleEnv.left1=[]
        simpleEnv.left2=[]
        ...
        simpleEnv.Target3=[]
        simpleEnv.Target4=[]
        simpleEnv.Target_left_car = ""
        simpleEnv.Target_right_car =""
        simpleEnv.last_action = [[0,0]]
        simpleEnv.Left_action = False
        simpleEnv.Right_action = False
        simpleEnv.rewards =[]
        simpleEnv.toward_space_reward=[]
        simpleEnv.toward_space_reward_long=[]
        simpleEnv.toward_space_reward_lat=[]
        simpleEnv.t_LC_start =0
        simpleEnv.initial_x = 0
        simpleEnv.initial_y = 0
        simpleEnv.initial_target_y =0
        simpleEnv.theta_i=0
        simpleEnv.final_x =9999
        simpleEnv.final_y = 0
        simpleEnv.step_num = 0
        simpleEnv.coefficient=[0,0,0,0]
        simpleEnv.x_final = 0
        simpleEnv.crosstrack_error_term=[]
        simpleEnv.heading_error_term=[]
        simpleEnv.delta_term = []
        simpleEnv.clearance_error_term =[]
        simpleEnv.velocity_error_term = []    
        simpleEnv.Time=[]
        simpleEnv.Right_SD = False
        simpleEnv.Left_SD = False
        simpleEnv.Front_SD = True
        simpleEnv.last_target_space =[0,0]
        simpleEnv.last_vehicles = ['','']
        simpleEnv.mode=[0,0,0,0,0,0]
        # simpleEnv.vehicles_tau=[random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27)]
        simpleEnv.vehicles_tau=[random.uniform(0.74,2.27),random.uniform(0.74,2.27),0.74,0.74,random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),random.uniform(0.74,2.27),0.74,0.74,0.74,0.74,0.74,0.74]
        # simpleEnv.vehicles_tau=[1.36]*20 ########### RL 검증
        simpleEnv.tau_random = [random.uniform(0,1),random.uniform(0,1),random.uniform(0,1),random.uniform(0,1),random.uniform(0,1),random.uniform(0,1),random.uniform(0,1),random.uniform(0,1)]
        simpleEnv.rearMaxSpeed = random.uniform(29.17,33.33)
        simpleEnv.rear_collision_mode =  random.randint(0, 1)
        simpleEnv.rear_tau_aggresive = random.uniform(0.1, 0.5)
        simpleEnv.rear_tau_safe = random.uniform(0.75, 2.75)
        simpleEnv.rear_sleep =  random.randint(0, 1)
        simpleEnv.None_collision = True
        # if self.episode!=0: 
        #     self.sumo.close()
        #     sys.stdout.flush()
    ...
    def save_action_csv(self,episode,name):
        ...
        df_ego_action = pd.DataFrame(simpleEnv.last_action,columns=col_action)
        # file_path = '/home/jewoo/Desktop/SUMO_RL-main/logs/sumo_origin_simple_env/ConfusionMatrix/data/'+name+'/action/'
        file_path = '/home/jewoo/Desktop/SUMO_RL_RainbowDQN/logs/sumo_origin_simple_env/data/'
    ...
    def save_csv(self,episode,name):
        ...
        df_ego = pd.DataFrame(simpleEnv.ego,columns=col)
        df_rear = pd.DataFrame(simpleEnv.rear,columns=col)
        df_forward = pd.DataFrame(simpleEnv.forward,columns=col)
        df_left0 = pd.DataFrame(simpleEnv.left0,columns=col)
        df_left1 = pd.DataFrame(simpleEnv.left1,columns=col)
        df_left2 = pd.DataFrame(simpleEnv.left2,columns=col)
        df_left3 = pd.DataFrame(simpleEnv.left3,columns=col)
        df_left4 = pd.DataFrame(simpleEnv.left4,columns=col)
        df_left5 = pd.DataFrame(simpleEnv.left5,columns=col)
        df_left6 = pd.DataFrame(simpleEnv.left6,columns=col)
        df_left7 = pd.DataFrame(simpleEnv.left7,columns=col)
        df_right0 = pd.DataFrame(simpleEnv.right0,columns=col)
        df_right1 = pd.DataFrame(simpleEnv.right1,columns=col)
        df_right2 = pd.DataFrame(simpleEnv.right2,columns=col)
        df_right3 = pd.DataFrame(simpleEnv.right3,columns=col)
        df_right4 = pd.DataFrame(simpleEnv.right4,columns=col)
        df_right5 = pd.DataFrame(simpleEnv.right5,columns=col)
        df_right6 = pd.DataFrame(simpleEnv.right6,columns=col)
        df_right7 = pd.DataFrame(simpleEnv.right7,columns=col)
        df_target1 = pd.DataFrame(simpleEnv.Target1,columns=col_target)
        df_target2 = pd.DataFrame(simpleEnv.Target2,columns=col_target)
        df_target3 = pd.DataFrame(simpleEnv.Target3,columns=col_target)
        df_target4 = pd.DataFrame(simpleEnv.Target4,columns=col_target)
        df_ego_control = pd.DataFrame(simpleEnv.ego_control,columns=col_control)
        df_ego_action = pd.DataFrame(simpleEnv.last_action,columns=col_action)
        df_reward = pd.DataFrame(simpleEnv.rewards,columns=col_reward)
        df_space_reward = pd.DataFrame(simpleEnv.toward_space_reward,columns=col_space_reward)
        df_space_reward_long = pd.DataFrame(simpleEnv.toward_space_reward_long,columns=col_space_reward)
        df_space_reward_lat = pd.DataFrame(simpleEnv.toward_space_reward_lat,columns=col_space_reward)
        ...
        # file_path = '/home/jewoo/Desktop/SUMO_RL-main/logs/sumo_origin_simple_env'+'/'+name+'/'+str(episode)
        file_path = '/home/jewoo/Desktop/SUMO_RL_RainbowDQN/logs/sumo_origin_simple_env/data/'+str(episode)
        os.makedirs(file_path, exist_ok=True)
        os.chdir(file_path)
    ...
    def __ego_vehicle_LC_start(self):
        ...
        if 'ego' in vehs:
            ...
            last_lane = simpleEnv.lane_buffer_ego
    ...
    def __ego_vehicle_LC_completed(self,action):
        vehs = traci.vehicle.getIDList()
        if 'ego' in vehs:
            if simpleEnv.ego_LC_start == True:
                ...
                # if ((traci.vehicle.getLaneIndex('ego')== 2 and (action == 1 or action == 3)) or (traci.vehicle.getLaneIndex('ego')== 0 and (action == 2 or action == 4))):
                    simpleEnv.ego_LC_completed =True
                    self.done = True
    ...
    def __get_a_x(self,id):
        dt = self.step_length
        if(len(simpleEnv.ego)!=0):
            v_x_cur = (traci.vehicle.getPosition(id)[0] - simpleEnv.ego[-1][2])/dt
            a_x_cur = (v_x_cur -simpleEnv.ego[-1][4])/dt
        else:
            v_x_cur=0
            a_x_cur=0
        return a_x_cur
    ...
    def __get_a_y(self,id):
        dt = self.step_length
        if(len(simpleEnv.ego)!=0):
            v_y_cur = (traci.vehicle.getPosition(id)[1] - simpleEnv.ego[-1][3])/dt
            a_y_cur = (v_y_cur -simpleEnv.ego[-1][5])/dt
        else:
            v_y_cur=0
            a_y_cur=0
        return a_y_cur
    ...
    def __set_a_y_possible(self,id,a_y_des): #id =='ego'
        ...
        if(len(simpleEnv.ego)!=0):
            v_y_cur = (traci.vehicle.getPosition(id)[1] - simpleEnv.ego[-1][3])/dt
            a_y_cur = (v_y_cur -simpleEnv.ego[-1][5])/dt
            # print("simpleEnv.ego[-1][5]): ",simpleEnv.ego[-1][5])
            # print("simpleEnv.ego[-1][3]: ",simpleEnv.ego[-1][3])
        else:
            v_y_cur=0
            a_y_cur=0
        #     print("simpleEnv.ego[-1][5]): None")
        #     print("simpleEnv.ego[-1][3]: None")
    ...
    def __current_goal_space_point(self,action): # action ==1 or action == 2 or action == 3 or action == 4
        ...
        if 'ego' in vehs:
            ...
            if len(simpleEnv.last_action) ==0:
                LLP = 0
            else:
        ...
        if 'ego' in vehs:
            ...
            if lane == 0 and LLP <0:
                if action == 5:
                    ...
                    traci.vehicle.unsubscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 300.0)
                    simpleEnv.last_target_space =left_target_space
                    simpleEnv.last_vehicles =vehicles
                    ...
                    print("Wrong action!!")
                    traci.vehicle.unsubscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 300.0)
                    simpleEnv.last_target_space =right_target_space
                    simpleEnv.last_vehicles =vehicles
                    return right_target_space, vehicles
                else:
                    traci.vehicle.unsubscribeContext(str(id), tc.CMD_GET_VEHICLE_VARIABLE, 300.0)
                    return simpleEnv.last_target_space , simpleEnv.last_vehicles
    ...
    def __ego_a_desire(self, v_c,v_t,c_d,c,space_num): # desire acceleration by ACC, v_c : ego_vehicle's velocity, v_t : target velocity, c_d : desire clearance, c = actual clearance
        ...
        Kd = 0.005
        Kd2 = 0.7
        simpleEnv.integral_clearance_e[space_num] += Ki*(c_d -c)*self.step_length
        simpleEnv.integral_velocity_e[space_num] += Ki2*(v_t-v_c)*self.step_length

        clearance_error = (c_d -c)
        velocity_error = (v_t-v_c)
        if len(simpleEnv.pre_error_velocity)>0 and len(simpleEnv.pre_error_velocity)>0:
            a_d = -P*self.__K1(v_c)*(c_d-c) - P2*self.__K2(v_c)*(v_t-v_c) -simpleEnv.integral_clearance_e[space_num] +simpleEnv.integral_velocity_e[space_num] -Kd*(clearance_error-simpleEnv.pre_error_clearance[space_num])/self.step_length +Kd2*(velocity_error-simpleEnv.pre_error_velocity[space_num])
        else:
            a_d = -P*self.__K1(v_c)*(c_d-c) - P2*self.__K2(v_c)*(v_t-v_c) -simpleEnv.integral_clearance_e[space_num] +simpleEnv.integral_velocity_e[space_num]
        ...
        if(a_d >3):
            ...
            return -5
        simpleEnv.pre_error_clearance[space_num] = (c_d -c)
        simpleEnv.pre_error_velocity[space_num] = (v_t-v_c)
    ...
    def __ego_a_desire2(self, v_c,v_t,c_d,c,space_num): # desire acceleration by ACC, v_c : ego_vehicle's velocity, v_t : target velocity, c_d : desire clearance, c = actual clearance
        ...
        # print('ego_v: ',v_c)
        # print('target_v: ',v_t)
        simpleEnv.clearance_error_term.append(-P*self.__K1(v_c)*(c_d-c))
        simpleEnv.velocity_error_term.append(-P2*self.__K2(v_c)*(v_t-v_c))
        simpleEnv.Time.append(traci.simulation.getTime())
        # plt.figure(1,figsize=(8,4))            
        # pylab.plot(simpleEnv.Time, simpleEnv.clearance_error_term, 'r',label='c_error_term')
        # pylab.plot(simpleEnv.Time, simpleEnv.velocity_error_term, 'b',label='vel_error_term')
    ...
    def __rear_vehicle_random_collsion(self, veh_id):
        ...
        # 충돌/비충돌 결정 (50% 확률)
        # 색상 설정
        if simpleEnv.rear_collision_mode == 1:  # 충돌 모드
            # 빨간색 계열
        ...
        if 'ego' in vehs:  # 선행 차량이 있을 때 ACC
            ...
            if simpleEnv.rear_collision_mode == 1:  # 충돌 모드
                # 위험한 tau 값 (0.1~0.5초)
                tau = simpleEnv.rear_tau_aggresive
                # 충돌 가능성 높은 차량은 로깅
                # print(f"[위험] 차량 {veh_id} - 충돌 모드, tau: {tau:.2f}초, 간격: {c_front:.2f}m")
                
            else:  # 안전 모드
                # 안전한 tau 값 (1.5~2.5초)
                tau = simpleEnv.rear_tau_safe
                # 안전 모드 차량 로깅
            ...
            if traci.simulation.getTime() > 20.00 and time_to_collision >3:
                self.done=True     
            ...
            # 비충돌 모드일 경우 감속 강화
            if simpleEnv.rear_collision_mode == 0 and time_to_collision < 2.0 and a_des<0:
                a_des = -9  # 풀브레이킹

             # 충돌 모드일 경우 감속 약화 (더 공격적인 주행)
            if simpleEnv.rear_collision_mode == 1 and a_des < 0:
                if simpleEnv.rear_sleep == 1:
                    a_des = 0
                else:
    ...
    def __setvehiclestate(self, id):
        ...
        if leader_info is not None:
            ...
            if simpleEnv.rear_collision_mode == 1:  # 충돌 시나리오
            ...
            # 로그 출력 (디버깅용)
            # print(f"Time: {sim_time:.2f}, Vehicle: {id}, Mode: {'Collision' if simpleEnv.rear_collision_mode==1 else 'Safe'}, " 
    ...
    def __set_ego_a_movetoXY(self,id,a_des): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if len(simpleEnv.ego)!=0:
            last_x = simpleEnv.ego[-1][2]
            last_y = simpleEnv.ego[-1][3]
            v_y_cur = (traci.vehicle.getPosition(id)[1] - simpleEnv.ego[-1][3])/dt
            a_y_cur = (v_y_cur -simpleEnv.ego[-1][5])/dt
        else:
            last_x = x_cur
        ...
        # print('y_cur: ',y_cur)
        # if len(simpleEnv.ego) != 0:
        #     if simpleEnv.ego[-1][8]<90:
        #         angle = simpleEnv.ego[-1][8] +4*dt
        #     elif simpleEnv.ego[-1][8]>90:
        #         angle = simpleEnv.ego[-1][8] -4*dt
        #     else:
        #         angle =90
        # else:
...
########################################################
        ...
        if(a_cur+jerk*dt<a_des):
            ...
            x_next = x_cur +v_next*dt 
            # return a_des
        # theta = np.arctan2(y_cur-simpleEnv.ego[-1][3],x_cur-simpleEnv.ego[-1][2])
        # if(y_cur-simpleEnv.ego[-1][3]>=0):
        #     angle = 90-theta*30
        # else:
        #     angle = 90-theta*30
        # print("angle:::::::::::::::::",angle)
    ...
    def __set_ego_a_movetoXY2(self,id,a_des,action): #return 값은 jerk 값을 고려한 현실적인 가속도 값을 반환한다. # SUMO에 가속도를 입력한다.
        ...
        if action == 1  and lane == 2 and x_cur > traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] + epsilon_d:
            target_lane_y = -1.6
        if action == 3 and lane == 2 and x_cur < traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] - epsilon_d:
            target_lane_y = -1.6
        if action == 2 and lane == 0 and x_cur > traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] + epsilon_d:
            target_lane_y = -8
        if action == 4 and lane == 0 and x_cur < traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] - epsilon_d:
            target_lane_y = -8
...
########################################################
    ...
    def state(self,id = 'ego'):
        states = []
        vehs = traci.vehicle.getIDList()
        if id in vehs and simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs:
            if traci.vehicle.getLeader(simpleEnv.Target_left_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==2:
                left_leader = traci.vehicle.getLeader('ego')[0]
            elif traci.vehicle.getLeader(simpleEnv.Target_left_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==1:
                left_leader = 'car.left'+str(int(simpleEnv.Target_left_car[-1])+1)
            else:
                left_leader = traci.vehicle.getLeader(simpleEnv.Target_left_car)[0]
            if traci.vehicle.getFollower(simpleEnv.Target_left_car)[0] =='ego' and traci.vehicle.getLaneIndex('ego')==2:
                left_follower = traci.vehicle.getFollower('ego')[0]
            elif traci.vehicle.getFollower(simpleEnv.Target_left_car)[0] =='ego' and traci.vehicle.getLaneIndex('ego')==1:
                left_follower = 'car.left'+str(int(simpleEnv.Target_left_car[-1])-1)
            else:
                left_follower = traci.vehicle.getFollower(simpleEnv.Target_left_car)[0]
            
            if traci.vehicle.getLeader(simpleEnv.Target_right_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==0:
                right_leader = traci.vehicle.getLeader('ego')[0]
            elif traci.vehicle.getLeader(simpleEnv.Target_right_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==1:
                right_leader = 'car.right'+str(int(simpleEnv.Target_right_car[-1])+1)
            else:
                right_leader = traci.vehicle.getLeader(simpleEnv.Target_right_car)[0]
            if traci.vehicle.getFollower(simpleEnv.Target_right_car)[0] =='ego' and traci.vehicle.getLaneIndex('ego')==0:
                right_follower = traci.vehicle.getFollower('ego')[0]
            elif traci.vehicle.getFollower(simpleEnv.Target_right_car)[0] =='ego' and traci.vehicle.getLaneIndex('ego')==1:
                right_follower = 'car.right'+str(int(simpleEnv.Target_right_car[-1])-1)
            else:
                right_follower = traci.vehicle.getFollower(simpleEnv.Target_right_car)[0]
            ...
            # if rear_rel_x == None and len(simpleEnv.rear) !=0:
            #     for sublist in reversed(simpleEnv.rear):
            #         if sublist[2] == None:
            #             pass
            #         else:
            ...
            left_rel_x, left_rel_y = np.asarray(traci.vehicle.getPosition(simpleEnv.Target_left_car)) - np.asarray(traci.vehicle.getPosition('ego'))
            left_v = traci.vehicle.getSpeed(simpleEnv.Target_left_car)
            ...
            right_rel_x, right_rel_y = np.asarray(traci.vehicle.getPosition(simpleEnv.Target_right_car)) - np.asarray(traci.vehicle.getPosition('ego'))
            right_v = traci.vehicle.getSpeed(simpleEnv.Target_right_car)
            ...
            s1_rel_x, s1_rel_y = ((traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_leader)[0])/2 - traci.vehicle.getPosition('ego')[0]), (traci.vehicle.getPosition(simpleEnv.Target_left_car)[1]- traci.vehicle.getPosition('ego')[1])
            s1_v = (traci.vehicle.getSpeed(simpleEnv.Target_left_car) + traci.vehicle.getSpeed(left_leader))/2
            s2_rel_x, s2_rel_y = ((traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_leader)[0])/2 - traci.vehicle.getPosition('ego')[0]), (traci.vehicle.getPosition(simpleEnv.Target_right_car)[1]- traci.vehicle.getPosition('ego')[1])
            s2_v = (traci.vehicle.getSpeed(simpleEnv.Target_right_car) + traci.vehicle.getSpeed(right_leader))/2
            s3_rel_x, s3_rel_y = ((traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] + traci.vehicle.getPosition(left_follower)[0])/2 - traci.vehicle.getPosition('ego')[0]), (traci.vehicle.getPosition(simpleEnv.Target_left_car)[1]- traci.vehicle.getPosition('ego')[1])
            s3_v = (traci.vehicle.getSpeed(simpleEnv.Target_left_car) + traci.vehicle.getSpeed(left_follower))/2
            s4_rel_x, s4_rel_y = ((traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] + traci.vehicle.getPosition(right_follower)[0])/2 - traci.vehicle.getPosition('ego')[0]), (traci.vehicle.getPosition(simpleEnv.Target_right_car)[1]- traci.vehicle.getPosition('ego')[1])
            s4_v = (traci.vehicle.getSpeed(simpleEnv.Target_right_car) + traci.vehicle.getSpeed(right_follower))/2
            ...
            space1_size = np.abs((traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] - traci.vehicle.getPosition(left_leader)[0]))
            space2_size = np.abs((traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] - traci.vehicle.getPosition(right_leader)[0]))
            space3_size = np.abs((traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] - traci.vehicle.getPosition(left_follower)[0]))
            space4_size = np.abs((traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] - traci.vehicle.getPosition(right_follower)[0]))
    ...
    def __toward_empty_space_reward(self, id, action):
        ...
        def reward_for_space(space, ego_pos, action, sd_lc_dist, dist_to_space, weight, action_codes):
            longitudinal_term = 1 if (
                (action_codes[0] == 'L' and action == 3 and (ego_pos[0] < traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] - self.SD_LC_with_respect_to_target_space('ego', 3, 'front') and ego_pos[0] > traci.vehicle.getPosition(left_follower)[0] + self.SD_LC_with_respect_to_target_space('ego', 3,'back'))) or
                (action_codes[0] == 'L' and action == 1 and (ego_pos[0] > traci.vehicle.getPosition(simpleEnv.Target_left_car)[0] + self.SD_LC_with_respect_to_target_space('ego', 1,'back') and ego_pos[0] < traci.vehicle.getPosition(left_leader)[0] - self.SD_LC_with_respect_to_target_space('ego', 1,'front'))) or
                (action_codes[0] == 'R' and action == 4 and (ego_pos[0] < traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] - self.SD_LC_with_respect_to_target_space('ego', 4,'front') and ego_pos[0] > traci.vehicle.getPosition(right_follower)[0] + self.SD_LC_with_respect_to_target_space('ego', 4,'back'))) or
                (action_codes[0] == 'R' and action == 2 and (ego_pos[0] > traci.vehicle.getPosition(simpleEnv.Target_right_car)[0] + self.SD_LC_with_respect_to_target_space('ego', 2,'back') and ego_pos[0] < traci.vehicle.getPosition(right_leader)[0] - self.SD_LC_with_respect_to_target_space('ego', 2,'front')))
            ) else 2 ** (-sd_lc_dist)
        ...
        if id in vehs and simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs:
            ...
            spaces = [
                [get_space_size(simpleEnv.Target_left_car, left_leader), get_midpoint_distance(simpleEnv.Target_left_car, left_leader, ego_pos), 'L1',get_target_car_forward_SD_LC_point_distance(simpleEnv.Target_left_car, ego_pos, self.SD_LC_with_respect_to_target_space('ego', 1,'back'))],
                [get_space_size(simpleEnv.Target_right_car, right_leader), get_midpoint_distance(simpleEnv.Target_right_car, right_leader, ego_pos), 'R1',get_target_car_forward_SD_LC_point_distance(simpleEnv.Target_left_car, ego_pos, self.SD_LC_with_respect_to_target_space('ego', 2,'back'))],
                [get_space_size(simpleEnv.Target_left_car, left_follower), get_midpoint_distance(simpleEnv.Target_left_car, left_follower, ego_pos), 'L2',get_target_car_rear_SD_LC_point_distance(simpleEnv.Target_left_car, ego_pos, self.SD_LC_with_respect_to_target_space('ego', 3,'front'))],
                [get_space_size(simpleEnv.Target_right_car, right_follower), get_midpoint_distance(simpleEnv.Target_right_car, right_follower, ego_pos), 'R2',get_target_car_rear_SD_LC_point_distance(simpleEnv.Target_left_car, ego_pos, self.SD_LC_with_respect_to_target_space('ego', 4,'front'))]
            ]
            spaces.sort()
        ...
        simpleEnv.toward_space_reward.append(space_reward)
        simpleEnv.toward_space_reward_long.append(space_reward_long)
        simpleEnv.toward_space_reward_lat.append(space_reward_lat)
    ...
    def __chosen_space_size_reward(self): #시뮬레이션 시작 처음 근접 좌우 차량 기준 앞뒤 공간으로 차로 변경 시 보상.
        vehs = traci.vehicle.getIDList()
        if simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs and 'ego' in vehs:
            if traci.vehicle.getLeader(simpleEnv.Target_left_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==2:
                left_leader = traci.vehicle.getLeader('ego')[0]
            elif traci.vehicle.getLeader(simpleEnv.Target_left_car)[0] == 'ego' and traci.vehicle.getLaneIndex('ego')==1:
                left_leader = 'car.left'+str(int(simpleEnv.Target_left_car[-1])+1)
            ...
            if traci.vehicle.getLaneIndex('ego') == 2:
                if traci.vehicle.getPosition('ego')[0] > traci.vehicle.getPosition(simpleEnv.Target_left_car)[0]:
                    ego_chosen_space = 'L1'
                else:
                    ego_chosen_space = 'L2'
            if traci.vehicle.getLaneIndex('ego') == 0:
                if traci.vehicle.getPosition('ego')[0] > traci.vehicle.getPosition(simpleEnv.Target_right_car)[0]:
                    ego_chosen_space = 'R1'
                else:
                    ego_chosen_space = 'R2'
    ...
    def SD_LC_with_respect_to_target_space(self,id,action,direction): #action 1~4 = target space 1~4, action 에 따라 SD_LC를 return /// direction은 해당 action의 SD 중 앞에 것, 뒤에 것중 하나를 지정.
        ...
        if simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs and 'ego' in vehs:  
            if action == 1 and direction == 'back':
                relative_velocity_term = max([traci.vehicle.getSpeed(simpleEnv.Target_left_car)-traci.vehicle.getSpeed(id),0] )*time_gap_LC_1
                minimum_clearance_trem = max([traci.vehicle.getSpeed(simpleEnv.Target_left_car)*time_gap_LC_2,c_LC])
                SD_LC_back = relative_velocity_term + minimum_clearance_trem
                # print('space1 SD_LC: ',SD_LC)
                # space1_size = (traci.vehicle.getPosition(traci.vehicle.getLeader(simpleEnv.Target_left_car)[0])[0] -traci.vehicle.getPosition(simpleEnv.Target_left_car)[0])
                ...
                relative_velocity_term = max([traci.vehicle.getSpeed(simpleEnv.Target_right_car)-traci.vehicle.getSpeed(id),0] )*time_gap_LC_1
                minimum_clearance_trem = max([traci.vehicle.getSpeed(simpleEnv.Target_right_car)*time_gap_LC_2,c_LC])
                SD_LC_back = relative_velocity_term + minimum_clearance_trem
                # print('space2 SD_LC: ',SD_LC)
                # space2_size = (traci.vehicle.getPosition(traci.vehicle.getLeader(simpleEnv.Target_right_car)[0])[0] -traci.vehicle.getPosition(simpleEnv.Target_right_car)[0])
                ...
                relative_velocity_term = max( [traci.vehicle.getSpeed(id) - traci.vehicle.getSpeed(simpleEnv.Target_left_car) ,0] )*time_gap_LC_1
                minimum_clearance_trem = max([traci.vehicle.getSpeed(id)*time_gap_LC_2,c_LC])
                SD_LC_front = relative_velocity_term +minimum_clearance_trem
                # print('space3 SD_LC: ',SD_LC)
                # space3_size = (traci.vehicle.getPosition(simpleEnv.Target_left_car)[0]-traci.vehicle.getPosition(traci.vehicle.getFollower(simpleEnv.Target_left_car)[0])[0])
                ...
                relative_velocity_term = max( [traci.vehicle.getSpeed(id) - traci.vehicle.getSpeed(simpleEnv.Target_right_car) ,0] )*time_gap_LC_1
                minimum_clearance_trem = max([traci.vehicle.getSpeed(id)*time_gap_LC_2,c_LC])
                SD_LC_front = relative_velocity_term +minimum_clearance_trem
                # print('space4 SD_LC: ',SD_LC)
                # space4_size = (traci.vehicle.getPosition(simpleEnv.Target_right_car)[0]-traci.vehicle.getPosition(traci.vehicle.getFollower(simpleEnv.Target_right_car)[0])[0])
    ...
    def __SD_check(self,action):
        if (action == 1 or action == 3) and simpleEnv.Left_action == True:
            action = 5
        if (action == 2 or action == 4) and simpleEnv.Right_action == True:
            action = 6
        id = 'ego'
        time_gap_LC_1 =1
        time_gap_LC_2 =0.5 
    ...
    #     # print('simpleEnv.Left_SD: ',simpleEnv.Left_SD )
    #     # print('simpleEnv.Right_SD: ',simpleEnv.Right_SD)
    #     # print('simpleEnv.Front_SD: ',simpleEnv.Front_SD)

    #     if len(traci.simulation.getCollidingVehiclesIDList()) !=0:
    #         # print('negative reward')
    #         log = traci.simulation.getCollisions()
    #         simpleEnv.None_collision = False
    ...
    #             #     reward +=1*w4
    #             if(simpleEnv.ego_LC_success ==False and rear_TTC<=13.2 and rear_clearance >=0 and traci.vehicle.getLaneIndex('ego')==1): #후방 차량 충돌 판단 가능 범위
    ...
    #         if(rear_TTC>13.2 and (action == 5) and simpleEnv.Front_SD): #safe state LK positive reward                    
    ...
    #         if len(simpleEnv.last_action)>0 and rear_TTC<13.2 and (((action == 1 or action == 3) and simpleEnv.Left_SD == True) or ((action == 2 or action ==  4) and simpleEnv.Right_SD == True)):                    
    #             if simpleEnv.Left_action == True and (action == 1 or action == 3):
    #                 if Lane == 1 and LLP >0:
    #                     reward+= LLP*w7
    ...
    #                     reward+=0
    #                     Reward.append(0)
    #             elif simpleEnv.Right_action == False and (action == 2 or action ==  4):
    #                 if Lane == 1 and LLP <0:
    #                     reward+= -LLP*w7
    #                     Reward.append(-LLP*w7)
    #                 elif Lane ==0 and LLP>0:
    ...
    #         if simpleEnv.None_collision == True: # if rear_clearance >=-30 episode ends
    #             reward += 30*w8
    #             Reward.append(30*w8)
    #         else:
    #             reward += 0
    ...
    #         Reward.append(0)
    #         reward +=0
    #     # print('simpleEnv.last_action: ',simpleEnv.last_action[-1][1])
    #     # print('simpleEnv.Left_action: ',simpleEnv.Left_action)
    #     # print('simpleEnv.Right_action: ',simpleEnv.Right_action)
    #     if ((Lane == 1 or Lane == 2) and action == 1 and simpleEnv.Left_action == True and simpleEnv.last_action[-1][1] == 1) or ((Lane == 1 or Lane == 2) and action == 3 and simpleEnv.Left_action == True and simpleEnv.last_action[-1][1] == 3) or ((Lane == 1 or Lane == 0) and action == 2 and simpleEnv.Right_action == True and simpleEnv.last_action[-1][1] == 2) or ((Lane == 1 or Lane == 0) and action == 4 and simpleEnv.Right_action == True and simpleEnv.last_action[-1][1] == 4):
    ...
    #     simpleEnv.rewards.append(Reward)
    #     return reward
    # def __reward(self, step_num, action):
    #     Reward = []
    #     Reward.append(traci.simulation.getTime())
    ...
    #         if traci.vehicle.getLaneIndex('ego') == 1:                
    #             if simpleEnv.ego_LC_success == False and rear_TTC <= 13.2 and rear_clearance >= 0 and traci.vehicle.getLaneIndex('ego') == 1:
    #                 reward -= 1.1**(10-rear_clearance) * w4
    #                 Reward.append(-1.1**(10-rear_clearance) * w4)
    #             else:
    #                 Reward.append(0)
    ...
    #         if rear_TTC > 13.2 and (action == 5) and simpleEnv.Front_SD:
    #             reward += (rear_TTC) * w6
    #             Reward.append((rear_TTC) * w6)
    #         else:
    #             Reward.append(0)
    ...
    #         if simpleEnv.None_collision == True:
    #             reward += 30 * w8
    #             Reward.append(30 * w8)
    #         else:
    #             reward += 0
    ...
    #     if ((Lane == 1 or Lane == 2) and action == 1 and simpleEnv.Left_action == True and simpleEnv.last_action[-1][1] == 1) or \
    #     ((Lane == 1 or Lane == 2) and action == 3 and simpleEnv.Left_action == True and simpleEnv.last_action[-1][1] == 3) or \
    #     ((Lane == 1 or Lane == 0) and action == 2 and simpleEnv.Right_action == True and simpleEnv.last_action[-1][1] == 2) or \
    #     ((Lane == 1 or Lane == 0) and action == 4 and simpleEnv.Right_action == True and simpleEnv.last_action[-1][1] == 4):
    #         reward += 1 * w9
    #         Reward.append(1 * w9)
    #     else:
    #         reward += 0
    ...
    def __reward(self, step_num, action):
            ...
            if rel_speed > 0 and rear_clearance > 0:
                ...
                if collision_imminent:
                    ...
                    if action != 5:  # LK가 아닌 경우
                        is_safe_LC = (action in [1, 3] and simpleEnv.Left_SD) or (action in [2, 4] and simpleEnv.Right_SD)
        ...
        if 'ego' in vehs and 'accel.rear' in vehs:           
            ...
            if traci.vehicle.getLaneIndex('ego') == 1:                
                ...
                if simpleEnv.ego_LC_success == False and collision_imminent and rear_clearance >= 0:
                    # 위험도에 따라 패널티 조정
            ...
            # 안전한 상황에서 LK에 대한 보상
            if not collision_imminent and (action == 5) and simpleEnv.Front_SD:
            ...
            # 차선 변경 진행에 대한 보상
            if len(simpleEnv.last_action) > 0 and collision_imminent and (
                ((action == 1 or action == 3) and simpleEnv.Left_SD) or 
                ((action == 2 or action == 4) and simpleEnv.Right_SD)
            ):                    
                if simpleEnv.Left_action and (action in [1, 3]):
                    if Lane == 1 and LLP > 0:
                        reward += LLP * w7
                        Reward.append(LLP * w7)
                    elif Lane == 2 and LLP < 0:
                        ...
                        reward += 0
                        Reward.append(0)
                elif not simpleEnv.Right_action and (action in [2, 4]):
                    if Lane == 1 and LLP < 0:
                        reward += -LLP * w7
                        Reward.append(-LLP * w7)
                    elif Lane == 0 and LLP > 0:
        ...
        if ((Lane == 1 or Lane == 2) and action == 1 and simpleEnv.Left_action and simpleEnv.last_action[-1][1] == 1) or \
        ((Lane == 1 or Lane == 2) and action == 3 and simpleEnv.Left_action and simpleEnv.last_action[-1][1] == 3) or \
        ((Lane == 1 or Lane == 0) and action == 2 and simpleEnv.Right_action and simpleEnv.last_action[-1][1] == 2) or \
        ((Lane == 1 or Lane == 0) and action == 4 and simpleEnv.Right_action and simpleEnv.last_action[-1][1] == 4):
    ...
    def LC_cubic_polynomial_path(self,x_initial,y_initial,x_final,y_final,rear_v_id,forward_v_id,theta_i,action):
        ...
        if self.gui_on:
            ...
            pylab.xlabel("x")
            pylab.ylabel("y")
            pylab.savefig("/home/jewoo/Desktop/SUMO_RL_RainbowDQN/path/graph.png")
            pylab.clf()
    ...
    def LC_Left(self,action,v_target,c_target):
        ...
        if('ego' in vehs):  
            ...
            if action == 5:
                ...
                if(len(simpleEnv.ego)!=0):
                    last_ego_x = simpleEnv.ego[-1][2]
                    last_ego_y = simpleEnv.ego[-1][3]
                    last_ego_v_y = simpleEnv.ego[-1][5]
                else:
                ...
                simpleEnv.last_target_space = xy
                simpleEnv.last_vehicles = ids
                target_x, target_y = xy[0],xy[1]
                rear_v_id,forward_v_id = ids
                if ego_x-simpleEnv.initial_x>=simpleEnv.final_x:
                    path_done = True
                else:
                    path_done = False
                ...
                if path_done == True and traci.vehicle.getLaneIndex('ego') ==1 and LLP <= 0 and LLP >= -0.4 and traci.vehicle.getAngle('ego')>=89.9:
                    print("---------------------------------------LC_succeed")
                    simpleEnv.initial_x = ego_x
                    simpleEnv.initial_y = ego_y
                    target_y = -1.6
                    simpleEnv.theta_i = (90-traci.vehicle.getAngle('ego'))*np.pi/180
                ...
                if simpleEnv.Left_action == False:
                    # print('simpleEnv.Left_action == False')
                    t = 0
                    simpleEnv.t_LC_start = traci.simulation.getTime()
                    x0=0
                    y0=0
                    simpleEnv.initial_x = ego_x
                    simpleEnv.initial_y = ego_y
                    simpleEnv.initial_target_y = target_y
                    simpleEnv.theta_i = (90-traci.vehicle.getAngle('ego'))*np.pi/180
                    simpleEnv.coefficient,simpleEnv.final_x,simpleEnv.final_y =self.LC_cubic_polynomial_path(simpleEnv.initial_x,simpleEnv.initial_y,target_x,target_y,rear_v_id,forward_v_id,simpleEnv.theta_i,action)
                else:
                    # print('simpleEnv.Left_action == True')
                    t= traci.simulation.getTime()-simpleEnv.t_LC_start
                    x0= simpleEnv.initial_x
                    y0= simpleEnv.initial_y
                    if path_done:                       
                        simpleEnv.coefficient = [target_y-ego_y,0,0,0]
                    if LC_succeed:                     
                        simpleEnv.coefficient,simpleEnv.final_x,simpleEnv.final_y =self.LC_cubic_polynomial_path(simpleEnv.initial_x,simpleEnv.initial_y,target_x,target_y,rear_v_id,forward_v_id,simpleEnv.theta_i,action)
                    
                x_local = ego_x - simpleEnv.initial_x
                # delta = self.stanley(simpleEnv.coefficient,traci.vehicle.getAngle('ego'),x_local,ego_v)
                x = ego_x -simpleEnv.initial_x
                y = simpleEnv.coefficient[2]*x**2+simpleEnv.coefficient[3]*x**3
                y_prime = 2*simpleEnv.coefficient[2]*x+3*simpleEnv.coefficient[3]*x**2
                y_pprime = 2*simpleEnv.coefficient[2]+6*simpleEnv.coefficient[3]*x
                ...
                simpleEnv.ego_control.append(control)
                theta = np.arctan2(ego_y-last_ego_y,ego_x-last_ego_x)
                theta +=delta
        ...
        if simpleEnv.final_x < ego_x-simpleEnv.initial_x:
            simpleEnv.Left_action = False
        else:
            simpleEnv.Left_action = True
        # print('simpleEnv.final_x(goal distance): ',simpleEnv.final_x)
        # print('ego_x-simpleEnv.initial_x(driven distance): ',ego_x-simpleEnv.initial_x)
    ...
    def LC_Right(self, action,v_target,c_target):
        ...
        if('ego' in vehs):  
            ...
            if action == 6:
                ...
                if simpleEnv.Right_action == False:
                    # print('simpleEnv.Right_action == False')
                    t = 0
                    simpleEnv.t_LC_start = traci.simulation.getTime()
                    simpleEnv.initial_x = ego_x
                    simpleEnv.initial_y = ego_y
                    ...
                    # print('simpleEnv.Right_action == True')
                    t= traci.simulation.getTime()-simpleEnv.t_LC_start                        
                    if path_done:
                        simpleEnv.coefficient = [target_y-ego_y,0,0,0]
                    if LC_succeed:                                                     
    ...
    def Target_point_visualization(self,TargetID,TargetPoint):
        epsilon = 0.5
        ego_inflation_x_back = 2.6
        ego_inflation_x_front = 3.05
        ego_inflation_y = 1.2
        x,y = TargetPoint
    ...
    def reset_left_right_target_vehicle(self):
        if traci.simulation.getTime() >= 0.03: # 기준이되는 옆차로 차량을 시뮬레이션 시작할 때 설정한다.(-> 후방 차량의 추돌 위험이 감지 될 때 마다 기준이 되는 옆차로 차량의 위치를 갱신하는게 맞다.)
            ...
            left_vehicles.sort() 
            right_vehicles.sort() 
            if simpleEnv.Target_left_car != left_vehicles[0][1]:
                if int(simpleEnv.Target_left_car[-1]) < int(left_vehicles[0][1][-1]):
                    self.remove_last_SD_visualization(traci.vehicle.getFollower(simpleEnv.Target_left_car)[0],simpleEnv.Target_left_car)
                else:
                    self.remove_last_SD_visualization(simpleEnv.Target_left_car,traci.vehicle.getLeader(simpleEnv.Target_left_car)[0])
            if simpleEnv.Target_right_car != right_vehicles[0][1]:
                if int(simpleEnv.Target_right_car[-1]) < int(right_vehicles[0][1][-1]):
                    self.remove_last_SD_visualization(traci.vehicle.getFollower(simpleEnv.Target_right_car)[0],simpleEnv.Target_right_car)
                else:
                    self.remove_last_SD_visualization(simpleEnv.Target_right_car,traci.vehicle.getLeader(simpleEnv.Target_right_car)[0])
            simpleEnv.Target_left_car = left_vehicles[0][1]
            simpleEnv.Target_right_car = right_vehicles[0][1]
            # print('New Target_left_car: ',simpleEnv.Target_left_car)
            # print('New Target_right_car: ',simpleEnv.Target_right_car)
    ...
    def RuleBasedResult(self):
        ...
        # s1_back = (self.vehicle_position(simpleEnv.Target_left_car)[0] + self.SD_LC_with_respect_to_target_space('ego',1,'back')) #자차와 왼쪽 차로 옆차량 SD_LC 위치
        # s2_back = (self.vehicle_position(simpleEnv.Target_right_car)[0] + self.SD_LC_with_respect_to_target_space('ego',2,'back'))   
        # s3_front = (self.vehicle_position(simpleEnv.Target_left_car)[0] - self.SD_LC_with_respect_to_target_space('ego',3,'front'))           
        # s4_front = (self.vehicle_position(simpleEnv.Target_right_car)[0] - self.SD_LC_with_respect_to_target_space('ego',4,'front')) 
        ...
        if simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs and 'ego' in vehs:
            ...
            space1 = (self.vehicle_position(simpleEnv.Target_left_car)[0] + self.vehicle_position(left_leader)[0])/2
            space2 = (self.vehicle_position(simpleEnv.Target_right_car)[0] + self.vehicle_position(right_leader)[0])/2
            space3 = (self.vehicle_position(simpleEnv.Target_left_car)[0] + self.vehicle_position(left_follower)[0])/2
            space4 = (self.vehicle_position(simpleEnv.Target_right_car)[0] + self.vehicle_position(right_follower)[0])/2
            Safe_space1 = s1_front-s1_back
            Safe_space2 = s2_front-s2_back
            Safe_space3 = s3_front-s3_back
            Safe_space4 = s4_front-s4_back
        else:
            s1_back= simpleEnv.S[1]
            s1_front = simpleEnv.S[5]
            s2_back= simpleEnv.S[2]
            s2_front= simpleEnv.S[6]
            s3_front= simpleEnv.S[3]
            s3_back= simpleEnv.S[7]
            s4_front= simpleEnv.S[4]
            s4_back= simpleEnv.S[8]
            space1 = simpleEnv.last_Space1
            space2 = simpleEnv.last_Space2
            space3 = simpleEnv.last_Space3
            space4 = simpleEnv.last_Space4
            Safe_space1 = simpleEnv.last_Safe_Space1
            Safe_space2 = simpleEnv.last_Safe_Space2
            Safe_space3 = simpleEnv.last_Safe_Space3
            Safe_space4 = simpleEnv.last_Safe_Space4
            left_leader, left_follower, right_leader, right_follower = self.reset_left_right_target_vehicle()
        ...
        ego_chosen_space = None
        if ego_x< self.vehicle_position(left_leader)[0] and ego_x>self.vehicle_position(simpleEnv.Target_left_car)[0] and traci.vehicle.getLaneIndex('ego') == 2:
            ego_chosen_space =1
        if ego_x< self.vehicle_position(right_leader)[0] and ego_x>self.vehicle_position(simpleEnv.Target_right_car)[0] and traci.vehicle.getLaneIndex('ego') == 0:
            ego_chosen_space =2
        if ego_x< self.vehicle_position(simpleEnv.Target_left_car)[0] and ego_x>self.vehicle_position(left_follower)[0] and traci.vehicle.getLaneIndex('ego') == 2:
            ego_chosen_space =3
        if ego_x< self.vehicle_position(simpleEnv.Target_right_car)[0] and ego_x>self.vehicle_position(right_follower)[0] and traci.vehicle.getLaneIndex('ego') == 0:
            ego_chosen_space =4
        #t1
        if ego_x > s1_front:
            simpleEnv.ego_at_Target[0] =False
            # print('s1_1st')
            ...
            simpleEnv.ego_at_Target[0] =True            
            # print('s1_2nd')
            v_t = (traci.vehicle.getSpeed(left_leader) +traci.vehicle.getSpeed(simpleEnv.Target_left_car))/2
            mid = (self.vehicle_position(left_leader)[0] +self.vehicle_position(simpleEnv.Target_left_car)[0])/2
            # if ego_x>mid:
            ...
            simpleEnv.ego_at_Target[0] =False
            # print('s1_3rd')
            v_t = traci.vehicle.getSpeed(simpleEnv.Target_left_car)
        ...
        ##t2
        if ego_x > s2_front:
            simpleEnv.ego_at_Target[1] =False
            # print('s2_1st')
            ...
            simpleEnv.ego_at_Target[1] =True
            # print('s2_2nd')
            v_t = (traci.vehicle.getSpeed(right_leader) +traci.vehicle.getSpeed(simpleEnv.Target_right_car))/2
            mid = (self.vehicle_position(right_leader)[0] +self.vehicle_position(simpleEnv.Target_right_car)[0])/2
            # if ego_x>mid:
            #     print('s2_2nd_1')
            #     s = ego_x - mid
            #     t2 = (-v_ego-math.sqrt(v_ego**2+2*ego_decel*s))/ego_decel
            ...
            simpleEnv.ego_at_Target[1] =False
            # print('s2_3rd')
            v_t = traci.vehicle.getSpeed(simpleEnv.Target_right_car)
            s = s2_back-ego_x
            # t2 = (-v_ego+math.sqrt(v_ego**2+2*ego_accel*s))/ego_accel
            v0 = np.abs(v_ego - v_t)
        ...
        #t3                  
        if ego_x > s3_front:
            simpleEnv.ego_at_Target[2] =False
            # print('s3_1st')      
            v_t = traci.vehicle.getSpeed(simpleEnv.Target_left_car)
            ...
            simpleEnv.ego_at_Target[2] =True
            # print('s3_2nd')
            v_t = (traci.vehicle.getSpeed(left_leader)+traci.vehicle.getSpeed(simpleEnv.Target_left_car))/2
            mid = (self.vehicle_position(simpleEnv.Target_left_car)[0] +self.vehicle_position(left_follower)[0])/2
            # if ego_x>mid:
        ...
        #t4    
        if ego_x > s4_front: 
            simpleEnv.ego_at_Target[3] =False
            # print('s4_1st')         
            v_t = traci.vehicle.getSpeed(simpleEnv.Target_right_car)
            ...
            simpleEnv.ego_at_Target[3] =True
            # print('s4_2nd')
            v_t = (traci.vehicle.getSpeed(right_leader)+traci.vehicle.getSpeed(simpleEnv.Target_right_car))/2
            mid = (self.vehicle_position(simpleEnv.Target_right_car)[0] +self.vehicle_position(right_follower)[0])/2
            # if ego_x>mid:
        ...
        #     x_final = 0
        # print('x_final: ',x_final)
        # print('simpleEnv.coefficient: ',simpleEnv.coefficient)
        for idx,value in enumerate(simpleEnv.ego_at_Target):
            if value == 1:
                if simpleEnv.x_final != 0:
                    T_LC_continue[idx],_ = self.calculate_LC_time(simpleEnv.coefficient, simpleEnv.x_final, v_ego)           
        ...
        for idx, time in enumerate(T_avoidance):
            ...
            # else:
            #     result[idx] = False
            if simpleEnv.mode[idx] == 1 and traci.vehicle.getLaneIndex('ego')!=1:
                result[idx] = True
        Rear_clearance_threshold = 100
        if rear_relative_distance>=Rear_clearance_threshold:
            return 5
    ...
    def confusion_matrix_data_collection(self,episode): #Collect the data of Rule best space and RL action 
        ...
        df_Rule_RL = pd.DataFrame(simpleEnv.Rule_RL,columns=col_Rule_RL)
        file_path = '/home/jewoo/Desktop/SUMO_RL_RainbowDQN/logs/ConfusionMatrix'
        os.makedirs(file_path, exist_ok=True)
        os.chdir(file_path)
    ...
    def step(self, action): #action-> 0: LK_const_vel, 1: LK_accel, 2: LK_decel
        # print('final_y: ', simpleEnv.final_y)
        ...
        # simpleEnv.vehicles_tau[1] =2.27 ##### RL 검증 space 3
        # simpleEnv.vehicles_tau[4] =2.27 #space1
        # simpleEnv.vehicles_tau[3] =2.27 #space 4
        # simpleEnv.vehicles_tau[11] =2.27 #space 2
        # vehs = traci.vehicle.getIDList()
        traci.simulationStep()
        ...
        if traci.simulation.getTime() > 0.03 and ('car.forward' not in vehs):
            ...
            # sys.stdout.flush()
        # print('simpleEnv.Target_right_car: ',simpleEnv.Target_right_car)
        # print('simpleEnv.Target_left_car: ',simpleEnv.Target_left_car)
        ...
        if traci.simulation.getTime() == 0.03: # 기준이되는 옆차로 차량을 시뮬레이션 시작할 때 설정한다.(-> 후방 차량의 추돌 위험이 감지 될 때 마다 기준이 되는 옆차로 차량의 위치를 갱신하는게 맞다.)
            ...
            simpleEnv.Target_left_car = left_vehicles[0][1]
            simpleEnv.Target_right_car = right_vehicles[0][1]
            print('Initial Target_left_car: ',simpleEnv.Target_left_car)
            print('Initial Target_right_car: ',simpleEnv.Target_right_car)
            traci.vehicle.unsubscribeContext('ego', tc.CMD_GET_VEHICLE_VARIABLE, 120.0)
        ...
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs): 
            ...
            c_LK = 2 #min clearance for lane keeping
            time_gap_LC_prepare = 0.74
            time_gap_LK_safe = 1.36  #95% percentile time_gap (most safe) //2.27 -> safe 1.36s -> 중간값   0.74s -> 5% persentile 가장 위험.
            time_gap_LK = 0.74
            # time_gap_LK = 1.2 #95% percentile time_gap (most safe) //2.27 -> safe 1.36s -> 중간값   0.74s -> 5% persentile 가장 위험.
            v_p = traci.vehicle.getSpeed(Leader_id)
            v_forward = traci.vehicle.getSpeed('car.forward')
            des_clearance = c_LK + time_gap_LK*v_p #앞차 속도에 따른 앞차와의 ACC 안전거리 (safe)
            LK_safe_clearance = c_LK + time_gap_LK_safe*v_p #앞차 속도에 따른 앞차와의 ACC 안전거리 (safe)
            des_clearance2 = c_LK + time_gap_LC_prepare*v_p #앞차 속도에 따른 앞차와의 ACC 안전거리 (aggressive)
            SD_LK_forward = c_LK + time_gap_LK*v_forward
            ...
            if self.gui_on:
                ...
                self.SD_visualization(simpleEnv.Target_left_car,self.SD_LC_with_respect_to_target_space('ego',1,'back'),'front','LC')
                self.SD_visualization(left_leader,self.SD_LC_with_respect_to_target_space('ego',1,'front'),'back','LC')
                self.SD_visualization(simpleEnv.Target_right_car,self.SD_LC_with_respect_to_target_space('ego',2,'back'),'front','LC')
                ...
                self.SD_visualization(simpleEnv.Target_left_car,self.SD_LC_with_respect_to_target_space('ego',3,'front'),'back','LC')
                self.SD_visualization(right_follower,self.SD_LC_with_respect_to_target_space('ego',4,'back'),'front','LC')
                self.SD_visualization(simpleEnv.Target_right_car,self.SD_LC_with_respect_to_target_space('ego',4,'front'),'back','LC')
            ...
            if action == 0:
                ...
                # if Anti_RVC_on == True: #LK_mode
                    # print('Anti_RVC_on')
                if LK_safe_clearance<=x_forward:
                ...
                if simpleEnv.mode[1] == 1:                   
                    if ego_x > s1_front -epsilon:
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s1_front - epsilon:                        
                            ...
                            # v_t = max(min(traci.vehicle.getSpeed(left_leader),traci.vehicle.getSpeed(Forward_id)),traci.vehicle.getSpeed(simpleEnv.Target_left_car))
                            v_t = traci.vehicle.getSpeed(left_leader)
                            c_t = s1_front - epsilon
                            if self.gui_on:
                                self.Target_point_visualization(left_leader,[s1_front - epsilon,self.vehicle_position(left_leader)[1]])
                        simpleEnv.Target1.append([traci.simulation.getTime(),1,s1_front,space1,v_t]) 
                        # print([0,1,0,0,0,0])
                        ...
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s1_back+epsilon:
                            ...
                            c_t = s1_back+epsilon
                            if self.gui_on:
                                self.Target_point_visualization(simpleEnv.Target_left_car,[s1_back+epsilon,self.vehicle_position(simpleEnv.Target_left_car)[1]])
                        simpleEnv.Target1.append([traci.simulation.getTime(),1,s1_back,space1,v_t]) 
                        # print([0,1,0,0,0,0])
                ...
                if simpleEnv.mode[1] == 2:
                    ...
                    if (traci.vehicle.getLaneIndex('ego') == 1):
                        if(self.vehicle_position('car.forward')[0]-ego_x<SD_LK_forward):
                            ...
                            v_t = (b*traci.vehicle.getSpeed(simpleEnv.Target_left_car)+a*traci.vehicle.getSpeed('car.forward'))/(a+b)
                            c_t = s1_back+epsilon+a/(a+b)*(self.vehicle_position('car.forward')[0]-SD_LK_forward -(s1_back+epsilon))- ego_x
                            if self.gui_on:
                                self.Target_point_visualization('weighted_point_with_Forward',[s1_back+epsilon+a/(a+b)*(self.vehicle_position('car.forward')[0]-SD_LK_forward -(s1_back+epsilon)),self.vehicle_position(simpleEnv.Target_left_car)[1]])
                        else:
                            a = ego_x - (s1_back+epsilon)
                            b = s1_front-epsilon - ego_x
                            v_t = (b*traci.vehicle.getSpeed(simpleEnv.Target_left_car)+a*traci.vehicle.getSpeed(left_leader))/(a+b)
                ...
                if simpleEnv.mode[2] == 1:  
                    if ego_x > s2_front -epsilon:
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s2_front - epsilon:
                            ...
                            # v_t = max(min(traci.vehicle.getSpeed(right_leader),traci.vehicle.getSpeed(Forward_id)),traci.vehicle.getSpeed(simpleEnv.Target_right_car))
                            v_t = traci.vehicle.getSpeed(right_leader)
                            c_t = s2_front - epsilon
                            if self.gui_on:
                                self.Target_point_visualization(right_leader,[s2_front - epsilon,self.vehicle_position(right_leader)[1]])
                        simpleEnv.Target2.append([traci.simulation.getTime(),2,s2_front,space2,v_t]) 
                        # print([0,0,1,0,0,0]) 
                        ...
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s2_back+epsilon:
                            ...
                            c_t = s2_back+epsilon
                            if self.gui_on:
                                self.Target_point_visualization(simpleEnv.Target_right_car,[s2_back+epsilon,self.vehicle_position(simpleEnv.Target_right_car)[1]])
                        simpleEnv.Target2.append([traci.simulation.getTime(),2,s2_back,space2,v_t]) 
                        # print([0,0,1,0,0,0])
                ...
                if simpleEnv.mode[2] == 2:
                    # print([0,0,2,0,0,0])
                    if (traci.vehicle.getLaneIndex('ego') == 1):
                        if(self.vehicle_position('car.forward')[0]-ego_x<SD_LK_forward):
                            a = ego_x -(s2_back + epsilon)
                            b = ego_x - (self.vehicle_position('car.forward')[0]-SD_LK_forward)
                            v_t = (b*traci.vehicle.getSpeed(simpleEnv.Target_right_car)+a*traci.vehicle.getSpeed('car.forward'))/(a+b)
                            c_t = s2_back+epsilon+a/(a+b)*(self.vehicle_position('car.forward')[0]-SD_LK_forward -(s2_back+epsilon))- ego_x
                            if self.gui_on:
                                self.Target_point_visualization('weighted_point_with_Forward',[s2_back+epsilon+a/(a+b)*(self.vehicle_position('car.forward')[0]-SD_LK_forward -(s2_back+epsilon)),self.vehicle_position(simpleEnv.Target_right_car)[1]])
                        else:
                            a = ego_x - (s2_back+epsilon)
                            b = s2_front-epsilon - ego_x
                            v_t = (b*traci.vehicle.getSpeed(simpleEnv.Target_right_car)+a*traci.vehicle.getSpeed(right_leader))/(a+b)
                ...
                if simpleEnv.mode[3] == 1:                   
                    if ego_x > s3_front -epsilon:
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s3_front - epsilon:
                            ...
                            if self.gui_on:
                                self.Target_point_visualization(left_leader,[s3_front - epsilon,self.vehicle_position(simpleEnv.Target_left_car)[1]])
                        simpleEnv.Target3.append([traci.simulation.getTime(),3,s3_front,space3,v_t]) 
                        # print([0,0,0,1,0,0])
                        ...
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s3_back+epsilon:
                            ...
                            v_t = max(min(traci.vehicle.getSpeed(simpleEnv.Target_left_car),traci.vehicle.getSpeed(Forward_id)),traci.vehicle.getSpeed(left_follower))
                            c_t = s3_back+epsilon
                            if self.gui_on:
                                self.Target_point_visualization(left_follower,[s3_back+epsilon,self.vehicle_position(left_follower)[1]])
                        simpleEnv.Target3.append([traci.simulation.getTime(),3,s3_back,space3,v_t]) 
                        # print([0,0,0,1,0,0])
                ...
                if simpleEnv.mode[3] == 2:
                    ...
                    if (traci.vehicle.getLaneIndex('ego') == 1):
                        if(self.vehicle_position('car.forward')[0]-ego_x<SD_LK_forward):
                            ...
                            v_t = (b*traci.vehicle.getSpeed(left_follower)+a*traci.vehicle.getSpeed(simpleEnv.Target_left_car))/(a+b)
                            c_t = s3_front - epsilon - ego_x
                            if self.gui_on:
                                self.Target_point_visualization(simpleEnv.Target_left_car,[s3_front - epsilon,self.vehicle_position(simpleEnv.Target_left_car)[1]])
                    else: # laneindex == 2 (TargetLane)
                        a = ego_x - (s3_back+epsilon)
                        b = s3_front-epsilon - ego_x
                        v_t = (b*traci.vehicle.getSpeed(left_follower)+a*traci.vehicle.getSpeed(simpleEnv.Target_left_car))/(a+b)
                ...
                if simpleEnv.mode[4] == 1:                   
                    if ego_x > s4_front -epsilon:
                        if self.vehicle_position(Forward_id)[0] - SD_LK_forward <= s4_front - epsilon:
                            ...
                            # v_t = max(min(traci.vehicle.getSpeed(simpleEnv.Target_right_car),traci.vehicle.getSpeed(Forward_id)),traci.vehicle.getSpeed(right_follower))
                            v_t = traci.vehicle.getSpeed(simpleEnv.Target_right_car)
                            c_t = s4_front - epsilon
                            if self.gui_on:
                                self.Target_point_visualization(simpleEnv.Target_right_car,[s4_front - epsilon,self.vehicle_position(simpleEnv.Target_right_car)[1]])
                        simpleEnv.Target4.append([traci.simulation.getTime(),4,s4_front,space4,v_t]) 
                        # print([0,0,0,0,1,0])
                        ...
                        simpleEnv.Target4.append([traci.simulation.getTime(),4,s4_back,space4,v_t]) 
                        # print([0,0,0,0,1,0])
                ...
                if simpleEnv.mode[4] == 2:
                    ...
                    if (traci.vehicle.getLaneIndex('ego') == 1):
                        if(self.vehicle_position('car.forward')[0]-ego_x<SD_LK_forward):
                            ...
                            v_t = (b*traci.vehicle.getSpeed(right_follower)+a*traci.vehicle.getSpeed(simpleEnv.Target_right_car))/(a+b)
            ...
            simpleEnv.last_Space1=space1
            simpleEnv.last_Space2=space2
            simpleEnv.last_Space3=space3
            simpleEnv.last_Space4=space4
            simpleEnv.S = [0,s1_back,s2_back,s3_front,s4_front,s1_front,s2_front,s3_back,s4_back]
        ...
        simpleEnv.Rule_RL.append([traci.simulation.getTime(),self.RuleBasedResult(),action])  
        ...
        for veh_id in vehs:
            ...
            if traci.vehicle.getRoadID(veh_id) == 'E5': 
                ...
                ID = veh_id.split('.')
                velocity_y=0
                if veh_id== 'ego' and not (len(simpleEnv.ego)==0):
                    velocity_y = (y1 - simpleEnv.ego[-1][3])/self.step_length
                if veh_id== 'car.left0' and not (len(simpleEnv.left0))==0:
                    velocity_y = (y1 - simpleEnv.left0[-1][3])/self.step_length
                if veh_id== 'car.left1' and not (len(simpleEnv.left1))==0:
                    velocity_y = (y1 - simpleEnv.left1[-1][3])/self.step_length
                if veh_id== 'accel.rear' and not (len(simpleEnv.rear))==0:
                    velocity_y = (y1 - simpleEnv.rear[-1][3])/self.step_length
                if veh_id== 'car.right0' and not (len(simpleEnv.right0))==0:
                    velocity_y = (y1 - simpleEnv.right0[-1][3])/self.step_length
                if veh_id== 'car.right1' and not (len(simpleEnv.right1))==0:
                    velocity_y = (y1 - simpleEnv.right1[-1][3])/self.step_length
                if veh_id== 'car.left2' and not (len(simpleEnv.left2))==0:
                    velocity_y = (y1 - simpleEnv.left2[-1][3])/self.step_length
                if veh_id== 'car.left3' and not (len(simpleEnv.left3))==0:
                    velocity_y = (y1 - simpleEnv.left3[-1][3])/self.step_length
                if veh_id== 'car.left4' and not (len(simpleEnv.left4))==0:
                    velocity_y = (y1 - simpleEnv.left4[-1][3])/self.step_length
                if veh_id== 'car.left5' and not (len(simpleEnv.left5))==0:
                    velocity_y = (y1 - simpleEnv.left5[-1][3])/self.step_length
                if veh_id== 'car.left6' and not (len(simpleEnv.left6))==0:
                    velocity_y = (y1 - simpleEnv.left6[-1][3])/self.step_length
                if veh_id== 'car.left7' and not (len(simpleEnv.left7))==0:
                    velocity_y = (y1 - simpleEnv.left7[-1][3])/self.step_length
                if veh_id== 'car.forward' and not (len(simpleEnv.forward))==0:
                    velocity_y = (y1 - simpleEnv.forward[-1][3])/self.step_length
                if veh_id== 'car.right2' and not (len(simpleEnv.right2))==0:
                    velocity_y = (y1 - simpleEnv.right2[-1][3])/self.step_length
                if veh_id== 'car.right3' and not (len(simpleEnv.right3))==0:
                    velocity_y = (y1 - simpleEnv.right3[-1][3])/self.step_length
                if veh_id== 'car.right4' and not (len(simpleEnv.right4))==0:
                    velocity_y = (y1 - simpleEnv.right4[-1][3])/self.step_length
                if veh_id== 'car.right5' and not (len(simpleEnv.right5))==0:
                    velocity_y = (y1 - simpleEnv.right5[-1][3])/self.step_length
                if veh_id== 'car.right6' and not (len(simpleEnv.right6))==0:
                    velocity_y = (y1 - simpleEnv.right6[-1][3])/self.step_length
                if veh_id== 'car.right7' and not (len(simpleEnv.right7))==0:
                    velocity_y = (y1 - simpleEnv.right7[-1][3])/self.step_length
                ...
                acceleration_y=0
                if veh_id== 'ego' and not (len(simpleEnv.ego)==0):
                    acceleration_y = (velocity_y - simpleEnv.ego[-1][5])/self.step_length
                if veh_id== 'car.left0' and not (len(simpleEnv.left0))==0:
                    acceleration_y = (velocity_y- simpleEnv.left0[-1][5])/self.step_length
                if veh_id== 'car.left1' and not (len(simpleEnv.left1))==0:
                    acceleration_y = (velocity_y - simpleEnv.left1[-1][5])/self.step_length
                if veh_id== 'accel.rear' and not (len(simpleEnv.rear))==0:
                    acceleration_y = (velocity_y - simpleEnv.rear[-1][5])/self.step_length
                if veh_id== 'car.right0' and not (len(simpleEnv.right0))==0:
                    acceleration_y = (velocity_y - simpleEnv.right0[-1][5])/self.step_length
                if veh_id== 'car.right1' and not (len(simpleEnv.right1))==0:
                    acceleration_y = (velocity_y - simpleEnv.right1[-1][5])/self.step_length
                if veh_id== 'car.left2' and not (len(simpleEnv.left2))==0:
                    acceleration_y = (velocity_y - simpleEnv.left2[-1][5])/self.step_length
                if veh_id== 'car.left3' and not (len(simpleEnv.left3))==0:
                    acceleration_y = (velocity_y - simpleEnv.left3[-1][5])/self.step_length
                if veh_id== 'car.left4' and not (len(simpleEnv.left4))==0:
                    acceleration_y = (velocity_y - simpleEnv.left4[-1][5])/self.step_length
                if veh_id== 'car.left5' and not (len(simpleEnv.left5))==0:
                    acceleration_y = (velocity_y - simpleEnv.left5[-1][5])/self.step_length
                if veh_id== 'car.left6' and not (len(simpleEnv.left6))==0:
                    acceleration_y = (velocity_y - simpleEnv.left6[-1][5])/self.step_length
                if veh_id== 'car.left7' and not (len(simpleEnv.left7))==0:
                    acceleration_y = (velocity_y - simpleEnv.left7[-1][5])/self.step_length
                if veh_id== 'car.forward' and not (len(simpleEnv.forward))==0:
                    acceleration_y = (velocity_y - simpleEnv.forward[-1][5])/self.step_length
                if veh_id== 'car.right2' and not (len(simpleEnv.right2))==0:
                    acceleration_y = (velocity_y - simpleEnv.right2[-1][5])/self.step_length
                if veh_id== 'car.right3' and not (len(simpleEnv.right3))==0:
                    acceleration_y = (velocity_y - simpleEnv.right3[-1][5])/self.step_length
                if veh_id== 'car.right4' and not (len(simpleEnv.right4))==0:
                    acceleration_y = (velocity_y - simpleEnv.right4[-1][5])/self.step_length
                if veh_id== 'car.right5' and not (len(simpleEnv.right5))==0:
                    acceleration_y = (velocity_y - simpleEnv.right5[-1][5])/self.step_length
                if veh_id== 'car.right6' and not (len(simpleEnv.right6))==0:
                    acceleration_y = (velocity_y - simpleEnv.right6[-1][5])/self.step_length
                if veh_id== 'car.right7' and not (len(simpleEnv.right7))==0:
                    acceleration_y = (velocity_y - simpleEnv.right7[-1][5])/self.step_length
                ...
                if(veh_id == 'ego'):
                    simpleEnv.ego.append(vehicle_state)
                    # print(vehicle_state)
                if(veh_id =='car.left0'):
                    simpleEnv.left0.append(vehicle_state)
                if(veh_id=='car.left1'):
                    simpleEnv.left1.append(vehicle_state)
                if(veh_id=='accel.rear'):
                    simpleEnv.rear.append(vehicle_state)
                if(veh_id=='car.right0'):
                    simpleEnv.right0.append(vehicle_state)
                if(veh_id=='car.right1'):
                    simpleEnv.right1.append(vehicle_state)
                if(veh_id=='car.left2'):
                    simpleEnv.left2.append(vehicle_state)
                if(veh_id=='car.left3'):
                    simpleEnv.left3.append(vehicle_state)
                if(veh_id=='car.left4'):
                    simpleEnv.left4.append(vehicle_state)
                if(veh_id=='car.left5'):
                    simpleEnv.left5.append(vehicle_state)
                if(veh_id=='car.left6'):
                    simpleEnv.left6.append(vehicle_state)
                if(veh_id=='car.left7'):
                    simpleEnv.left7.append(vehicle_state)
                if(veh_id=='car.forward'):
                    simpleEnv.forward.append(vehicle_state)
                if(veh_id=='car.right2'):
                    simpleEnv.right2.append(vehicle_state)
                if(veh_id=='car.right3'):
                    simpleEnv.right3.append(vehicle_state)
                if(veh_id=='car.right4'):
                    simpleEnv.right4.append(vehicle_state)    
                if(veh_id=='car.right5'):
                    simpleEnv.right5.append(vehicle_state)   
                if(veh_id=='car.right6'):
                    simpleEnv.right6.append(vehicle_state)    
                if(veh_id=='car.right7'):
                    simpleEnv.right7.append(vehicle_state)   
            ...
            if veh_id == 'ego':
                if self.__ego_vehicle_LC_start() and traci.simulation.getTime()>=0.01:
                    ...
                    simpleEnv.ego_LC_start =True
                    traci.vehicle.setLaneChangeMode('ego',0)
                self.__ego_vehicle_LC_completed(action) # LC completed checking
                simpleEnv.lane_buffer_ego = traci.vehicle.getLaneIndex('ego') # ego_car lane buffer  
            ...
            if traci.vehicle.getRoadID(veh_id) == 'E5' and veh_id[0] == 'a':
                ...
                # if traci.vehicle.getSpeed('accel.rear') <= simpleEnv.rearMaxSpeed:
            ...
            if (traci.vehicle.getRoadID(veh_id) == 'E5' and veh_id[0] == 'c'):
                ...
                if veh_id == 'car.left0':
                    tau = simpleEnv.vehicles_tau[0]                    
                elif veh_id =='car.left1':
                    tau = simpleEnv.vehicles_tau[1]                    
                elif veh_id =='car.right0':
                    tau = simpleEnv.vehicles_tau[2]                    
                elif veh_id =='car.right1':
                    tau = simpleEnv.vehicles_tau[3]                    
                elif veh_id =='car.left2':
                    tau = simpleEnv.vehicles_tau[4]               
                elif veh_id =='car.left3':
                    tau = simpleEnv.vehicles_tau[5]                    
                elif veh_id =='car.left4':
                    tau = simpleEnv.vehicles_tau[6]
                elif veh_id =='car.left5':
                    tau = simpleEnv.vehicles_tau[7]
                elif veh_id =='car.left6':
                    tau = simpleEnv.vehicles_tau[8]
                elif veh_id =='car.left7':
                    tau = simpleEnv.vehicles_tau[9]
                elif veh_id =='car.forward':
                    tau = simpleEnv.vehicles_tau[10]                    
                elif veh_id =='car.right2':
                    tau = simpleEnv.vehicles_tau[11]                    
                elif veh_id =='car.right3':
                    tau = simpleEnv.vehicles_tau[12]                    
                elif veh_id =='car.right4':
                    tau = simpleEnv.vehicles_tau[13]
                elif veh_id =='car.right5':
                    tau = simpleEnv.vehicles_tau[14]
                elif veh_id =='car.right6':
                    tau = simpleEnv.vehicles_tau[15]
                elif veh_id =='car.right7':
                    tau = simpleEnv.vehicles_tau[16]
                else:
                    tau = 1.36
                ...
                if traci.vehicle.getLeader(veh_id) is not None: # 선행 차량이 있을 때 ACC
                    Leader_id,c_front = traci.vehicle.getLeader(veh_id)
                    v_preceding= traci.vehicle.getSpeed(Leader_id)
                    if Leader_id == 'ego'and simpleEnv.tau_random[int(veh_id[-1])] <= 0.5: # 50% 확률로 앞으로 끼어드는 ego vehicle 양보 x
                        tau = 0
                    c_desire = c0+tau*v_preceding
        ...
        # print(simpleEnv.step_num)
        ##### update Left Right Target Car #####
        # if 'ego'in vehs and simpleEnv.Target_left_car in vehs and simpleEnv.Target_right_car in vehs and traci.simulation.getTime()>0.03:
            
        #     last_target_left =''    
        #     last_target_right =''  
        #     if (traci.vehicle.getPosition('ego')[0]> traci.vehicle.getPosition(traci.vehicle.getLeader(simpleEnv.Target_left_car)[0])[0]):
        #         last_target_left = simpleEnv.Target_left_car
        #         simpleEnv.Target_left_car = traci.vehicle.getLeader(simpleEnv.Target_left_car)[0]                
        #         print('New Target_left_car: ',simpleEnv.Target_left_car)                
        #     if (traci.vehicle.getPosition('ego')[0]<traci.vehicle.getPosition(traci.vehicle.getFollower(simpleEnv.Target_left_car)[0])[0]):
        #         last_target_left = simpleEnv.Target_left_car
        #         simpleEnv.Target_left_car = traci.vehicle.getFollower(simpleEnv.Target_left_car)[0]                
        #         print('New Target_left_car: ',simpleEnv.Target_left_car)  
                
        #     if(traci.vehicle.getPosition('ego')[0]> traci.vehicle.getPosition(traci.vehicle.getLeader(simpleEnv.Target_right_car)[0])[0]):
        #         last_target_right = simpleEnv.Target_right_car
        #         simpleEnv.Target_right_car = traci.vehicle.getLeader(simpleEnv.Target_right_car)[0]
        #         print('New Target_right_car: ',simpleEnv.Target_right_car) 
        #     if(traci.vehicle.getPosition('ego')[0]<traci.vehicle.getPosition(traci.vehicle.getFollower(simpleEnv.Target_right_car)[0])[0]):
        #         last_target_right = simpleEnv.Target_right_car
        #         simpleEnv.Target_right_car = traci.vehicle.getFollower(simpleEnv.Target_right_car)[0]
        #         print('New Target_right_car: ',simpleEnv.Target_right_car)
        #     if last_target_left != '':
        #         polygon_ids = traci.polygon.getIDList()
        #         for polygon_id in polygon_ids:  
        ...
        if traci.simulation.getTime() > 0.03 and ('backupcar.forward' not in vehs):
            self.done=True
        if traci.simulation.getTime() > 0.03 and ('car.left7' not in vehs):
            self.done=True
        if traci.simulation.getTime() > 0.03 and ('car.right7' not in vehs):
            self.done=True 
        
        if('ego' in vehs and 'car.forward' in vehs and 'accel.rear' in vehs):
            nextstate = self.state('ego')            
            reward = self.__reward(simpleEnv.step_num, action) # rear vehicle collision : -10, collision caused by ego : -20, LC_succeed_with_biggest_space : +20, LC_succeed_with_smaller_space : +10, step*-0.01
        else:
            nextstate = simpleEnv.last_state ## it might wrong code
            self.done = True 
            reward = self.__reward(simpleEnv.step_num, action)
            print("else!!!!!!!!!!!!!!!!!!!!!!!")
            
                
        simpleEnv.step_num +=1    
        self.ego_collision_happened()
        simpleEnv.last_action.append([traci.simulation.getTime(),action])
