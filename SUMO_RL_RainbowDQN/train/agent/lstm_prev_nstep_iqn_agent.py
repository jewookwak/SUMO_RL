# train/lstm_prev_nstep_iqn_agent.py
import os
import numpy as np
...
class LSTMPrevNStepIQNAgent:
    """LSTM-DQN agent with Implicit Quantile Networks, optimized for previous state sequence."""
    ...
    def get_action(self, state_memory):
        ...
        Returns:
            int: Selected action
        """
        if np.all(state_memory[0] == 5):  # 시퀀스가 비어있는지 확인 (5로 초기화된 경우)
            return 5
    ...
    def append_sample(self, state, action, reward, next_state, done):
        ...
        if n_step_sample:
            ...
            # 여기에 디버그 코드 추가 - 메모리 샘플 정보 출력
            memory_size = self.memory.tree.n_entries
            
            # 메모리 크기가 특정 값일 때만 출력 (너무 많은 출력을 피하기 위해)
            if memory_size in [10, 50, 100, 500, 1000] or memory_size % 1000 == 0:
                print(f"\n===== 메모리 크기: {memory_size} =====")
                print(f"행동(Action): {action}")
                print(f"보상(Reward): {n_step_reward:.4f}")
                print(f"종료(Done): {final_done}")
                print(f"TD 오류(Error): {td_error:.4f}")
                
                print(f"\n상태(State) 정보:")
                print(f"- 형태(Shape): {initial_state.shape}")
                print(f"- 값 범위: {np.min(initial_state):.4f} ~ {np.max(initial_state):.4f}")
                print(f"- 평균: {np.mean(initial_state):.4f}")
                
                # 시퀀스 데이터 샘플 출력
                if len(initial_state.shape) > 1 and initial_state.shape[0] > 1:
                    print("\n상태 첫 행과 마지막 행 샘플:")
                    print(f"첫 행: {initial_state[0][:5]} ... (나머지 생략)")
                    print(f"마지막 행: {initial_state[-1][:5]} ... (나머지 생략)")
                    
                    # 각 차원별 통계 (첫 몇 개 차원만)
                    if initial_state.shape[1] > 1:
                        print("\n차원별 통계 (첫 5개 차원):")
                        dim_count = min(5, initial_state.shape[1])
                        for dim in range(dim_count):
                            values = initial_state[:, dim]
                            print(f"차원 {dim}: 평균={np.mean(values):.4f}, 최소={np.min(values):.4f}, 최대={np.max(values):.4f}")
                
                print("\n다음 상태(Next State) 정보:")
                print(f"- 형태(Shape): {final_next_state.shape}")
                print(f"- 값 범위: {np.min(final_next_state):.4f} ~ {np.max(final_next_state):.4f}")
                
                # 현재 Q값과 타겟 Q값 출력
                print(f"\nQ값 정보:")
                print(f"- 현재 Q값(선택한 행동): {current_q:.4f}")
                print(f"- 타겟 Q값: {target:.4f}")
                
                print("==============================\n")
            
            # 랜덤 샘플링으로 가끔 출력 (약 1%)
            elif np.random.random() < 0.01:
                print(f"\n===== 메모리 랜덤 샘플 (크기: {memory_size}) =====")
                print(f"행동: {action}, 보상: {n_step_reward:.4f}, 종료: {final_done}")
                print(f"TD 오류: {td_error:.4f}")
                print(f"상태 형태: {initial_state.shape}, 다음 상태 형태: {final_next_state.shape}")
                print("==============================\n")
    ...
    def save_model(self, path):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        self.model.save_weights(os.path.join(path, 'lstm_prev_nstep_iqn_model.h5'))
        
    def load_model(self, path):
        """Load model weights from file."""
        self.model.load_weights(os.path.join(path, 'lstm_prev_nstep_iqn_model.h5'))
        self.update_target_model()