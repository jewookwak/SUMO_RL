...
class LSTMAutoencoderTrainer:
    def __init__(self, env, config, agent=None):
        self.env = env
        self.config = config
    ...
    def pretrain_autoencoders(self, train_data, epochs=50):
        ...
        for epoch in range(epochs):
            ...
            # 10 에포크마다 모델 저장
            if (epoch + 1) % 10 == 0:
                save_dir = "pretrained_autoencoders"
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                
                # 오토인코더 가중치 저장
                self._save_autoencoder_weights(save_dir, epoch)
    ...
    def train(self):
        """메인 훈련 루프"""
        # 1. 초기 단계: 오토인코더 사전 훈련
        if not self.is_autoencoder_pretrained:
            print("===== 초기 단계: 오토인코더 사전 훈련 =====")
            # 훈련 데이터 생성
    ...
    def _save_weights(self, best=False):
        ...
        agent_type = "lstm_autoencoder_external" if self.using_external_agent else "lstm_autoencoder_dqn"
    ...
    def _save_autoencoder_weights(self, save_dir, epoch):
        """오토인코더 가중치만 별도 저장"""
        # 차량 오토인코더 저장
