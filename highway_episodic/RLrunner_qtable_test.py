...
def get_options():
    ...
    optParser.add_option("-N","--episodenum", 
                        default=5000, help="numer of episode to run qlenv")
    optParser.add_option("--nogui", action="store_true",
                        default=False, help="run commandline version of sumo")
    options, args = optParser.parse_args()
    return options 

def TD_Error(env,q,prev_state, action, reward, nextstate, alpha, gamma): ## need to check
...
def rl_run(sumoBinary, episodenum,net, sumocfg,step_length, veh):  
    ...
    q  = np.load("/home/mds/Desktop/highway_episodic/q_table2/q_table 2024-02-01_04:31:37.980873.npy")
    ...
    for episode in range(episodenum):
        ...
        while (env.done) == False:
            ...
            if action == 0 or action ==1 or action ==2:
                ...
                TD_error = TD_Error(env,q,state, action, reward, nextstate, alpha, gamma)  ### 3 ###
                state = nextstate
