...
from DQN_env_LSTM_reduced_action_const_LC import rlEnv
...
class DQNAgent:
    def __init__(self, state_size, action_size): 
        ...
        self.model.load_weights("/home/mds/Desktop/highway_episodic/DQN/DQN_LSTM/save_model/trained_with_LSTM_reduced_action_const_LC_(3200episodes)/model")

    # 입실론 탐욕 정책으로 행동 선택
    def get_action(self, state, last_action):
        state = np.expand_dims(state, axis=0)  # state를 3차원 텐서로 변환
        if last_action == 1 and env.Left_action:
